...
import 'package:spots/core/models/entity_knot.dart';
...
class EntityKnotService {
  ...
  }) : _personalityKnotService = personalityKnotService ?? PersonalityKnotService();
  ...
  Map<String, double> _extractEventProperties(ExpertiseEvent event) {
    ...
    properties['timeProximity'] = (1.0 / (daysUntilEvent.abs() + 1.0)).clamp(0.0, 1.0);
    
    // Location encoding (if available)
    if (event.latitude != null && event.longitude != null) {
      properties['latitude'] = (event.latitude! + 90.0) / 180.0; // Normalize to [0, 1]
      properties['longitude'] = (event.longitude! + 180.0) / 360.0; // Normalize to [0, 1]
    }
  ...
  Map<String, double> _extractPlaceProperties(Spot spot) {
    final properties = <String, double>{};
    ...
    // Location encoding
    properties['latitude'] = (spot.latitude + 90.0) / 180.0; // Normalize to [0, 1]
    properties['longitude'] = (spot.longitude + 180.0) / 360.0; // Normalize to [0, 1]
  ...
  Map<String, double> _extractBusinessProperties(BusinessAccount business) {
    final properties = <String, double>{};
    
    // Business type encoding
    properties['businessType'] = business.businessType.hashCode.toDouble() % 1000 / 1000.0;
    ...
    properties['businessName'] = business.name.hashCode.toDouble() % 1000 / 1000.0;
  ...
  Map<String, double> _analyzeEventPropertyEntanglement(Map<String, double> properties) {
  ...
  Map<String, double> _analyzePlacePropertyEntanglement(Map<String, double> properties) {
    // Same approach as events
    return _analyzeEventPropertyEntanglement(properties);
  }

  /// Analyze property entanglement for businesses
  Map<String, double> _analyzeBusinessPropertyEntanglement(Map<String, double> properties) {
    // Same approach as events
    return _analyzeEventPropertyEntanglement(properties);
  }
  ...
  List<double> _createBraidDataFromEntanglement(Map<String, double> correlations) {
  ...
  String _getEntityId(dynamic entity) {
    ...
    if (entity is ExpertiseEvent) return entity.id;
    if (entity is Spot) return entity.id;
    if (entity is BusinessAccount) return entity.id;
    return 'unknown';
  }
}
