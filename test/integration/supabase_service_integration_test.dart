...
void main() {
  group('SupabaseService Integration Tests', () {
    ...
    group('Spot Operations', () {
      test('should create spots with required and optional fields, and retrieve spots', () async {
        if (!supabaseAvailable) {
          return; // Skip if Supabase not available
        }
        ...
        try {
          ...
          if (service.currentUser != null) {
            final userSpots = await service.getSpotsByUser(service.currentUser!.id);
    ...
    group('Spot List Operations', () {
      test('should create spot lists, retrieve all lists, and add spots to lists', () async {
        if (!supabaseAvailable) {
          return; // Skip if Supabase not available
        }
      ...
      test('should update and retrieve user profiles', () async {
        ...
        try {
          if (service.currentUser != null) {
            ...
            final getResult = await service.getUserProfile(service.currentUser!.id);
