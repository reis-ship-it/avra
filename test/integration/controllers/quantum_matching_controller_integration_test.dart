  ...
  group('QuantumMatchingController Integration Tests', () {
    ...
    group('execute', () {
      test('should successfully perform matching with event', () async {
        ...
        expect(result.matchingResult!.quantumCompatibility, lessThanOrEqualTo(1.0));
        expect(result.matchingResult!.locationCompatibility, greaterThanOrEqualTo(0.0));
        expect(result.matchingResult!.locationCompatibility, lessThanOrEqualTo(1.0));
        expect(result.matchingResult!.timingCompatibility, greaterThanOrEqualTo(0.0));
        expect(result.matchingResult!.timingCompatibility, lessThanOrEqualTo(1.0));
        expect(result.matchingResult!.entities.length, greaterThanOrEqualTo(2)); // User + Event
        ...
        expect(result.matchingResult!.metadata!['entityCount'], greaterThanOrEqualTo(2));
      });
      ...
      test('should successfully perform matching with multiple entities', () async {
        // Arrange
        final user = IntegrationTestHelpers.createUserWithLocalExpertise(
          id: 'user-4',
          category: 'Coffee',
      ...
      test('should return privacy-protected agentId in metadata', () async {
        ...
        expect(result.matchingResult!.metadata, isNotNull);
        expect(result.matchingResult!.metadata!['agentId'], isNotNull);
        expect(result.matchingResult!.metadata!['agentId'], isNot(equals(user.id))); // Should be different from user ID
      });
      ...
      test('should calculate compatibility scores correctly', () async {
        ...
        expect(result.matchingResult!.compatibility, inInclusiveRange(0.0, 1.0));
        expect(result.matchingResult!.quantumCompatibility, inInclusiveRange(0.0, 1.0));
        expect(result.matchingResult!.locationCompatibility, inInclusiveRange(0.0, 1.0));
        expect(result.matchingResult!.timingCompatibility, inInclusiveRange(0.0, 1.0));
