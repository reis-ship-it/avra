...
void main() {
  ...
  group('Phase 8: End-to-End Onboarding Workflow', () {
    test('Complete workflow: Onboarding → PersonalityProfile → PreferencesProfile', () async {
      // Arrange
      ...
      expect(preferencesProfile.categoryPreferences['Food & Drink'], greaterThan(0.7));
      expect(preferencesProfile.localityPreferences['San Francisco, CA'], equals(0.8));
      ...
      final savedPersonality = await personalityLearning.getCurrentPersonality(userId);
      expect(savedPersonality, isNotNull);
      expect(savedPersonality!.agentId, equals(agentId));

      final savedPreferences = await preferencesService.getPreferencesProfile(agentId);
    ...
    test('Workflow: Onboarding data → Both profiles → Quantum state conversion', () async {
      // Arrange
      ...
      final personalityQuantum = personalityProfile.dimensions; // Already quantum-enabled
      expect(personalityQuantum, isNotEmpty);
      expect(personalityQuantum.values.every((v) => v >= 0.0 && v <= 1.0), isTrue);
    ...
    test('Workflow: Profiles persist across app restarts', () async {
      ...
      final reloadedPersonality = await personalityLearning.getCurrentPersonality(userId);
      final reloadedPreferences = await preferencesService.getPreferencesProfile(agentId);
      final reloadedOnboarding = await onboardingService.getOnboardingData(userId);
    ...
    test('Workflow: PreferencesProfile updates without overwriting PersonalityProfile', () async {
      // Arrange
      ...
      await onboardingService.saveOnboardingData(userId, onboardingData);
      final initialPersonality = await personalityLearning.initializePersonalityFromOnboarding(
        userId,
        onboardingData: {'preferences': onboardingData.preferences},
        socialMediaData: null,
      );
      final initialPreferences = await preferencesService.initializeFromOnboarding(
        onboardingData,
      );
      ...
      final currentPersonality = await personalityLearning.getCurrentPersonality(userId);
      expect(currentPersonality, isNotNull);
      expect(currentPersonality!.agentId, equals(initialPersonality.agentId));
      expect(currentPersonality.dimensions, equals(initialPersonality.dimensions));

      // Step 4: Verify PreferencesProfile updated
      final currentPreferences = await preferencesService.getPreferencesProfile(agentId);
      ...
      expect(currentPreferences.source, equals('hybrid')); // Changed from 'onboarding'
    });
    ...
    test('Workflow: Complete data flow with all onboarding factors', () async {
      ...
      expect(personalityProfile.dimensions['exploration_eagerness'] ?? 0.0, greaterThan(0.0));
      // Multiple favorite places should increase exploration
      expect(personalityProfile.dimensions['location_adventurousness'] ?? 0.0, greaterThan(0.0));
      // Multiple preferences should influence various dimensions
      expect(personalityProfile.archetype, isNotEmpty);

      // Assert - Verify PreferencesProfile reflects onboarding data
      expect(preferencesProfile.categoryPreferences.length, greaterThan(4)); // Multiple categories
      expect(preferencesProfile.categoryPreferences['Coffee'], equals(0.8));
      expect(preferencesProfile.categoryPreferences['Craft Beer'], equals(0.8));
      expect(preferencesProfile.categoryPreferences['Food & Drink'], greaterThan(0.7));
      expect(preferencesProfile.localityPreferences['Portland, OR'], equals(0.8));
      expect(preferencesProfile.source, equals('onboarding'));
