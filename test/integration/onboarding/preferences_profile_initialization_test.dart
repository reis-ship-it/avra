...
void main() {
  ...
  group('PreferencesProfile Initialization Integration', () {
    test('should initialize PreferencesProfile from onboarding data', () async {
      ...
      // Act
      final profile = await preferencesService.initializeFromOnboarding(onboardingData);
    ...
    test('should save and load PreferencesProfile from storage', () async {
      ...
      final savedProfile = await preferencesService.initializeFromOnboarding(onboardingData);

      // Load from storage
      final loadedProfile = await preferencesService.getPreferencesProfile(agentId);

      // Assert
      expect(loadedProfile, isNotNull);
      expect(loadedProfile!.agentId, equals(savedProfile.agentId));
      expect(loadedProfile.categoryPreferences, equals(savedProfile.categoryPreferences));
      expect(loadedProfile.localityPreferences, equals(savedProfile.localityPreferences));
      expect(loadedProfile.source, equals(savedProfile.source));
    });

    test('should not overwrite learned PreferencesProfile with onboarding data', () async {
      // Arrange
      final preferencesService = di.sl<PreferencesProfileService>();
      final agentIdService = di.sl<AgentIdService>();
      ...
      // Act
      final result = await preferencesService.initializeFromOnboarding(onboardingData);
      ...
      expect(result.categoryPreferences['Activities'], isNull); // Not from onboarding
    });
    ...
    test('should update PreferencesProfile with learned data', () async {
      ...
      final initialProfile = await preferencesService.initializeFromOnboarding(onboardingData);
      ...
      final loadedProfile = await preferencesService.getPreferencesProfile(agentId);
      expect(loadedProfile, isNotNull);
      expect(loadedProfile!.source, equals('hybrid')); // Changed from 'onboarding' to 'hybrid'
