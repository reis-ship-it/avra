  ...
  group('Community Event Integration Tests', () {
    ...
    group('Community Event Upgrade Flow', () {
      test('should upgrade community event to local expert event', () async {
        ...
        var currentEvent = (await communityEventService.getCommunityEventById(event.id))!;
        ...
        currentEvent = (await communityEventService.getCommunityEventById(event.id))!;
      ...
      test('should preserve event history during upgrade', () async {
        ...
        expect(upgradedEvent.createdAt.difference(event.createdAt).inMilliseconds, lessThan(100),
    ...
    group('Community Events in Event Browse', () {
      test('should display community events in browse', () async {
        ...
        expect(event1Found, isTrue, reason: 'Event1 should be found in community events');
        expect(event2Found, isTrue, reason: 'Event2 should be found in community events');
      });
      ...
      test('should filter community events by category', () async {
        ...
        expect(initialEvents, isEmpty, reason: 'Service should be empty before test');
        ...
        final allEventsBeforeFilter = await communityEventService.getCommunityEvents();
        ...
        expect(coffeeEvents.any((e) => e.id == coffeeEvent.id), isTrue,
            reason: 'Coffee event should be found when filtering by Coffee category');
        // Verify food event is NOT in coffee results
        expect(coffeeEvents.any((e) => e.id == foodEvent.id), isFalse,
            reason: 'Food event should not be found when filtering by Coffee category');
        ...
        expect(foodEvents.any((e) => e.id == foodEvent.id), isTrue,
            reason: 'Food event should be found when filtering by Food category');
        ...
        expect(foodEvents.any((e) => e.id == coffeeEvent.id), isFalse,
            reason: 'Coffee event should not be found when filtering by Food category');
      });
      ...
      test('should filter community events by host', () async {
        ...
        final allEventsBeforeHostFilter = await communityEventService.getCommunityEvents();
        expect(allEventsBeforeHostFilter.any((e) => e.id == host1Event.id), isTrue,
            reason: 'Host1 event should exist before filtering');
        expect(allEventsBeforeHostFilter.any((e) => e.id == host2Event.id), isTrue,
