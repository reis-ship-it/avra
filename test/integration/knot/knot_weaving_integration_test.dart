...
void main() {
  group('Knot Weaving Integration Tests', () {
    ...
    group('End-to-End Braiding Workflow', () {
      test('should complete full workflow: generate knots → weave → store → retrieve', () async {
        // Step 1: Generate personality knots
        final profile1 = PersonalityProfile.initial('agent-1', userId: 'user-1');
        final profile2 = PersonalityProfile.initial('agent-2', userId: 'user-2');
        ...
        final retrieved = await knotStorageService.getBraidedKnot('connection-1');
      ...
      test('should create different braided knots for different relationship types', () async {
        final profile1 = PersonalityProfile.initial('agent-3', userId: 'user-3');
        final profile2 = PersonalityProfile.initial('agent-4', userId: 'user-4');
        ...
        expect(friendshipBraid.relationshipType, equals(RelationshipType.friendship));
        expect(romanticBraid.relationshipType, equals(RelationshipType.romantic));
      ...
      test('should calculate compatibility before weaving', () async {
        final profile1 = PersonalityProfile.initial('agent-5', userId: 'user-5');
        final profile2 = PersonalityProfile.initial('agent-6', userId: 'user-6');

        final knot1 = await personalityKnotService.generateKnot(profile1);
        final knot2 = await personalityKnotService.generateKnot(profile2);

        // Calculate compatibility
        final compatibility = await knotWeavingService.calculateWeavingCompatibility(
          knotA: knot1,
          knotB: knot2,
        );
      ...
      test('should handle multiple braided knots for same connection', () async {
        final profile1 = PersonalityProfile.initial('agent-7', userId: 'user-7');
        final profile2 = PersonalityProfile.initial('agent-8', userId: 'user-8');
        ...
        // Retrieve both
        final retrieved1 = await knotStorageService.getBraidedKnot('connection-2');
        final retrieved2 = await knotStorageService.getBraidedKnot('connection-3');

        expect(retrieved1, isNotNull);
        expect(retrieved2, isNotNull);
        expect(retrieved1?.relationshipType, equals(RelationshipType.friendship));
        expect(retrieved2?.relationshipType, equals(RelationshipType.collaborative));
      });

      test('should delete braided knot when connection is removed', () async {
        final profile1 = PersonalityProfile.initial('agent-9', userId: 'user-9');
        final profile2 = PersonalityProfile.initial('agent-10', userId: 'user-10');
