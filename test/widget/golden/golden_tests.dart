...
/// Configures golden test behavior for consistent visual regression testing
GoldenToolkitConfiguration get goldenToolkitConfiguration {
  return GoldenToolkitConfiguration(
    enableRealShadows: true, // Use realistic shadows for better visual accuracy
    fileNameFactory: (name) =>
        '$name.png', // Standard PNG format for golden files
  );
}
      ...
      group('Golden File Tests - Visual Regression Protection', () {
        group('Authentication Pages Golden Tests', () {
          ...
          testGoldens('Login page with error state', (tester) async {
            // Arrange
            final mockAuthBloc =
                MockBlocFactory.createErrorAuthBloc('Invalid credentials');
            final widget = WidgetTestHelpers.createTestableWidget(
              child: const LoginPage(),
              authBloc: mockAuthBloc,
            );
        ...
        group('Lists Page Golden Tests', () {
          ...
          testGoldens('Lists page with data', (tester) async {
            ...
            final mockListsBloc =
                MockBlocFactory.createLoadedListsBloc(testLists);
            final widget = WidgetTestHelpers.createTestableWidget(
              child: const ListsPage(),
              listsBloc: mockListsBloc,
            );
          ...
          testGoldens('Lists page error state', (tester) async {
            // Arrange
            final mockListsBloc =
                MockBlocFactory.createErrorListsBloc('Network error');
            final widget = WidgetTestHelpers.createTestableWidget(
              child: const ListsPage(),
              listsBloc: mockListsBloc,
            );
        ...
        group('Dialog Golden Tests', () {
          testGoldens('Age verification dialog', (tester) async {
            // Arrange - Show dialog directly in widget tree for golden test
            final widget = WidgetTestHelpers.createTestableWidget(
              child: Builder(
                builder: (context) {
            ...
            await tester.pump(); // Allow postFrameCallback to execute
