  ...
  group('GooglePlacesDataSourceNewImpl', () {
    ...
    group('searchPlaces - Request Building', () {
      test('should build correct request body with query only', () async {
        ...
        final body = json.decode(request!['body'] as String) as Map<String, dynamic>;
      ...
      test('should build correct headers with API key and field mask', () async {
        const query = 'restaurant';
        fakeHttpClient.setResponse(http.Response(
          '{"places": []}',
          200,
        ...
        expect(headers['X-Goog-FieldMask']!.contains('places.displayName'), isTrue);
        expect(headers['X-Goog-FieldMask']!.contains('places.location'), isTrue);
      });
    ...
    group('searchPlaces - JSON Parsing', () {
      ...
      test('should handle missing optional fields gracefully', () async {
        ...
        expect(spot.address, anyOf(isNull, isEmpty)); // Can be null or empty string
      ...
      test('should map Google API types to SPOTS categories correctly', () async {
        final testCases = [
          {'query': 'restaurant', 'types': ['restaurant'], 'expectedCategory': 'Food'},
          {'query': 'cafe', 'types': ['cafe'], 'expectedCategory': 'Food'},
          {'query': 'attraction', 'types': ['tourist_attraction'], 'expectedCategory': 'Attractions'},
          {'query': 'museum', 'types': ['museum'], 'expectedCategory': 'Attractions'},
          {'query': 'shopping', 'types': ['shopping_mall'], 'expectedCategory': 'Shopping'},
          {'query': 'nightclub', 'types': ['night_club'], 'expectedCategory': 'Nightlife'},
          {'query': 'hotel', 'types': ['lodging'], 'expectedCategory': 'Stay'},
          {'query': 'unknown', 'types': ['unknown_type'], 'expectedCategory': 'Other'},
        ];
    ...
    group('searchPlaces - Cache Behavior', () {
      test('should return cached results on second call with same parameters', () async {
        const query = 'restaurant';
        final mockResponse = http.Response(
          json.encode({
            'places': [
    ...
    group('searchPlaces - Error Handling', () {
      ...
      test('should return empty list on API error response', () async {
        const query = 'restaurant';
        fakeHttpClient.setResponse(http.Response(
          json.encode({'error': {'message': 'API key invalid'}}),
          403,
        ));
    ...
    group('getPlaceDetails', () {
      test('should normalize place ID format (add places/ prefix if missing)', () async {
    ...
    group('searchNearbyPlaces', () {
      test('should build correct nearby search request body', () async {
        ...
        final locationRestriction = body['locationRestriction'] as Map<String, dynamic>;
...
class _FakeGooglePlacesCacheService implements GooglePlacesCacheService {
  ...
  @override
  Future<Map<String, dynamic>?> getCachedPlaceDetails(String placeId) async => null;
