...
void main() {
  group('SocialMediaDataCollectionController', () {
    ...
    group('collectAllData', () {
      ...
      test('should successfully collect data from multiple platforms', () async {
        // Arrange
        ...
        verify(mockSocialMediaService.fetchProfileData(googleConnection)).called(1);
        verify(mockSocialMediaService.fetchProfileData(instagramConnection)).called(1);
        verify(mockSocialMediaService.fetchGooglePlacesData(googleConnection)).called(1);
      });

      test('should return success with empty data when no connections exist', () async {
        // Arrange
        when(mockSocialMediaService.getActiveConnections(userId))
            .thenAnswer((_) async => []);
      ...
      test('should continue on failure per platform', () async {
        ...
        when(mockSocialMediaService.fetchGooglePlacesData(googleConnection))
            .thenAnswer((_) async => {'places': [], 'reviews': [], 'photos': []});
        ...
        // Assert
        expect(result.isSuccess, isTrue); // Should succeed even if one platform fails
        expect(result.profileData, isNotNull);
      ...
      test('should merge Google Places data into profile', () async {
        ...
        expect(result.isSuccess, isTrue);
        expect(result.profileData, isNotNull);
        final googleProfile = result.profileData!['google'] as Map<String, dynamic>;
    ...
    group('fetchPlatformSpecificData', () {
      test('should fetch Google Places data for Google platform', () async {
        ...
        verify(mockSocialMediaService.fetchGooglePlacesData(connection)).called(1);
      });
