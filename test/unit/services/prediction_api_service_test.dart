...
void main() {
  group('PredictionAPIService', () {
    ...
    group('getMeaningfulConnectionPredictions', () {
      test('should return meaningful connection predictions for an event', () async {
        // Arrange
        ...
        when(mockEventService.getEventById('event-1')).thenAnswer((_) async => event);
        when(mockAgentIdService.getUserAgentId('user-1')).thenAnswer((_) async => 'agent-1');
        when(mockAgentIdService.getUserAgentId('user-2')).thenAnswer((_) async => 'agent-2');
        ...
        final connection1 = result.predictedConnections.firstWhere((c) => c.agentId == 'agent-1');
      ...
      test('should throw ArgumentError when event not found', () async {
        ...
        final tAtomic = AtomicTimestamp.now(precision: TimePrecision.millisecond);
        when(mockAtomicClock.getAtomicTimestamp()).thenAnswer((_) async => tAtomic);
        when(mockEventService.getEventById('non-existent')).thenAnswer((_) async => null);

        // Act & Assert
        expect(
          () => service.getMeaningfulConnectionPredictions(eventId: 'non-existent'),
      ...
      test('should handle errors when converting userId to agentId gracefully', () async {
        // Arrange
        final tAtomic = AtomicTimestamp.now(precision: TimePrecision.millisecond);
        when(mockAtomicClock.getAtomicTimestamp()).thenAnswer((_) async => tAtomic);
        ...
        when(mockEventService.getEventById('event-1')).thenAnswer((_) async => event);
        when(mockAgentIdService.getUserAgentId('user-1')).thenThrow(Exception('Service error'));
        when(mockAgentIdService.getUserAgentId('user-2')).thenAnswer((_) async => 'agent-2');
    ...
    group('getVibeEvolutionPredictions', () {
      test('should return vibe evolution predictions for a user', () async {
        ...
        // Mock vibe analyzer
        final userVibe = UserVibe.fromPersonalityProfile(userId, personalityProfile.dimensions);
      ...
      test('should throw ArgumentError when personality profile not found', () async {
        // Arrange
        final tAtomic = AtomicTimestamp.now(precision: TimePrecision.millisecond);
        when(mockAtomicClock.getAtomicTimestamp()).thenAnswer((_) async => tAtomic);
      ...
      test('should integrate knot string predictions when available', () async {
        ...
        when(mockAgentIdService.getUserAgentId(userId)).thenAnswer((_) async => agentId);
        ...
        verify(mockStringService!.predictFutureKnot(agentId, event.startTime)).called(1);
      });
    });
    ...
    group('getUserJourneyPredictions', () {
      test('should return user journey predictions', () async {
        ...
        expect(journeyStep.predictedTransformativeImpact, equals(0.68)); // 0.85 * 0.8
    ...
    group('encryptAndTransmitPrediction', () {
      test('should encrypt and transmit prediction via Signal Protocol + AI2AI mesh', () async {
        // Arrange
