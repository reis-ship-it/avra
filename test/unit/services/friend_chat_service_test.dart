...
// Mock classes
class MockMessageEncryptionService extends Mock implements MessageEncryptionService {}
  ...
  group('FriendChatService Tests', () {
    ...
    group('Send Message', () {
      test('should send encrypted message to friend', () async {
        ...
        // Act
        final chatMessage = await service.sendMessage(testUserId, testFriendId, message);
        ...
        verify(() => mockEncryptionService.encrypt(message, testChatId)).called(1);
      });
      
      test('should generate consistent chat ID regardless of user order', () async {
        // Arrange
        const message = 'Test message';
        
        // Act - send from user to friend
        final message1 = await service.sendMessage(testUserId, testFriendId, message);
        
        // Act - send from friend to user (reverse order)
        final message2 = await service.sendMessage(testFriendId, testUserId, message);
      ...
      test('should create unique message IDs', () async {
        ...
        // Act
        final message1 = await service.sendMessage(testUserId, testFriendId, message);
        final message2 = await service.sendMessage(testUserId, testFriendId, message);
    ...
    group('Conversation History', () {
      test('should retrieve conversation history', () async {
        ...
        // Act
        final history = await service.getConversationHistory(testUserId, testFriendId);
      ...
      test('should return empty list for non-existent conversation', () async {
        // Act
        final history = await service.getConversationHistory(testUserId, 'nonexistent_friend');
    ...
    group('Friends Chat List', () {
      ...
      test('should skip friends with no conversation', () async {
        ...
        // Act
        final chatList = await service.getFriendsChatList(testUserId, friendIds);
    ...
    group('Mark as Read', () {
      test('should mark messages as read', () async {
        ...
        final history = await service.getConversationHistory(testUserId, testFriendId);
        final unreadMessages = history.where((msg) => 
          !msg.isRead && msg.recipientId == testUserId
        );
      ...
      test('should only mark messages from specific friend', () async {
        ...
        // Assert
        final history1 = await service.getConversationHistory(testUserId, testFriendId);
        final history2 = await service.getConversationHistory(testUserId, otherFriend);
        
        expect(history1.first.isRead, isTrue);
        expect(history2.first.isRead, isFalse); // Other friend's message still unread
      });
    ...
    group('Unread Count', () {
      test('should calculate total unread count across all friends', () async {
        ...
        // Act
        final totalUnread = await service.getTotalUnreadCount(testUserId, friendIds);
      ...
      test('should return zero if no unread messages', () async {
        ...
        // Act
        final totalUnread = await service.getTotalUnreadCount(testUserId, [testFriendId]);
