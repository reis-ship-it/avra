...
class MockLanguagePatternLearningService extends Mock implements LanguagePatternLearningService {}
...
void main() {
  ...
  group('PersonalityAgentChatService Tests', () {
    ...
    setUp(() async {
      ...
      when(() => mockLanguageLearningService.analyzeMessage(any(), any(), any()))
    ...
    group('Chat Functionality', () {
      test('should process chat message and return agent response', () async {
        ...
        verify(() => mockEncryptionService.encrypt(any(), any())).called(greaterThan(0));
      ...
      test('should include conversation history in LLM context', () async {
        ...
        verify(() => mockLLMService.chat(
          messages: any(named: 'messages', that: predicate((List<ChatMessage> msgs) {
    ...
    group('Language Learning Integration', () {
      ...
      test('should get language style summary for context', () async {
        // Arrange
        when(() => mockLanguageLearningService.getLanguageStyleSummary(any()))
            .thenAnswer((_) async => 'User\'s Communication Style:\n- Vocabulary: cool, awesome\n- Formality: low');
        
        // Act
        await service.chat(testUserId, 'Test message');
        
        // Assert
        verify(() => mockLanguageLearningService.getLanguageStyleSummary(testUserId))
            .called(1);
      });
    });
    ...
    group('Conversation History', () {
      ...
      test('should retrieve conversation history', () async {
        ...
        // Assert
        expect(history.length, greaterThanOrEqualTo(2)); // At least 2 user messages + 2 agent responses
      });
      ...
      test('should decrypt messages when retrieving history', () async {
        ...
        verify(() => mockEncryptionService.decrypt(any(), any())).called(greaterThan(0));
      });
    });
    ...
    group('Encryption', () {
      ...
      test('should decrypt messages for display', () async {
        ...
        verify(() => mockEncryptionService.decrypt(any(), any())).called(greaterThanOrEqualTo(1));
      });
    });
