...
void main() {
  ...
  group('ActionHistoryService', () {
    ...
    group('Action Storage', () {
      test('should store actions with intent and result in chronological order, not store failed actions, and enforce maximum history size', () async {
        ...
        final result = ActionResult.success(message: 'Spot created successfully', data: {'spotId': 'spot123'}, intent: intent);
        ...
        final intent1 = CreateSpotIntent(name: 'Spot 1', description: 'First spot', latitude: 0.0, longitude: 0.0, category: 'Test', userId: 'user123', confidence: 0.9);
        final intent2 = CreateListIntent(title: 'List 1', description: 'First list', userId: 'user123', confidence: 0.8);
        await service.addAction(intent: intent1, result: ActionResult.success(intent: intent1));
        await Future.delayed(const Duration(milliseconds: 10));
        await service.addAction(intent: intent2, result: ActionResult.success(intent: intent2));
        ...
        final failureResult = ActionResult.failure(error: 'Failed to create spot', intent: intent);
        ...
        for (int i = 0; i < 55; i++) {
          final intent = CreateSpotIntent(name: 'Spot $i', description: 'Test', latitude: 0.0, longitude: 0.0, category: 'Test', userId: 'user123', confidence: 0.9);
          await service.addAction(intent: intent, result: ActionResult.success(intent: intent));
          await Future.delayed(const Duration(milliseconds: 1));
        }
        final history4 = await service.getHistory();
        expect(history4.length, lessThanOrEqualTo(50));
        expect((history4.first.intent as CreateSpotIntent).name, equals('Spot 54'));
      });
    });
    
    group('Action Retrieval', () {
      test('should retrieve all actions, return empty list when none stored, get recent actions with limit, and get undoable actions', () async {
    ...
    group('Undo Functionality', () {
      test('should check if action can be undone, undo actions by ID or last action, return false for already undone actions, and handle errors (no actions, not found, already undone)', () async {
        // Test business logic: undo functionality with validation and error handling
        final intent = CreateSpotIntent(name: 'Test Spot', description: 'Test', latitude: 0.0, longitude: 0.0, category: 'Test', userId: 'user123', confidence: 0.9);
        await service.addAction(intent: intent, result: ActionResult.success(intent: intent));
        final history1 = await service.getHistory();
        ...
        final intent1 = CreateSpotIntent(name: 'Spot 1', description: 'Test', latitude: 0.0, longitude: 0.0, category: 'Test', userId: 'user123', confidence: 0.9);
        final intent2 = CreateListIntent(title: 'List 1', description: 'Test', userId: 'user123', confidence: 0.8);
        await service.addAction(intent: intent1, result: ActionResult.success(intent: intent1));
        await Future.delayed(const Duration(milliseconds: 10));
        await service.addAction(intent: intent2, result: ActionResult.success(intent: intent2));
        final undoLastResult = await service.undoLastAction();
        expect(undoLastResult.success, isTrue);
        final history2 = await service.getHistory();
        expect(history2.first.intent.type, equals('create_list'));
        expect(history2.first.isUndone, isTrue);
    ...
    group('Edge Cases', () {
      test('should handle storage errors, empty history, undoable action filtering, and different action types gracefully', () async {
        ...
        final successIntent = CreateSpotIntent(name: 'Success Spot', description: 'Test', latitude: 0.0, longitude: 0.0, category: 'Test', userId: 'user123', confidence: 0.9);
        await service.addAction(intent: successIntent, result: ActionResult.success(intent: successIntent));
        await Future.delayed(const Duration(milliseconds: 50));
        final history1 = await service.getHistory();
        await service.undoAction(history1.first.id);
        await Future.delayed(const Duration(milliseconds: 50));
        final undoable = await service.getUndoableActions();
        expect(undoable.length, lessThan(history1.length)); // Some actions undone

        final spotIntent = CreateSpotIntent(name: 'Test Spot', description: 'Test', latitude: 0.0, longitude: 0.0, category: 'Test', userId: 'user123', confidence: 0.9);
        final listIntent = CreateListIntent(title: 'Test List', description: 'Test', userId: 'user123', confidence: 0.8);
        final addIntent = AddSpotToListIntent(spotId: 'spot1', listId: 'list1', userId: 'user123', confidence: 0.7);
        await service.addAction(intent: spotIntent, result: ActionResult.success(intent: spotIntent));
        await service.addAction(intent: listIntent, result: ActionResult.success(intent: listIntent));
        await service.addAction(intent: addIntent, result: ActionResult.success(intent: addIntent));
        await Future.delayed(const Duration(milliseconds: 50));
        final history2 = await service.getHistory();
