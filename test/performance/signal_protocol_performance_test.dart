...
void main() {
  group('Signal Protocol Performance Benchmarks', () {
    ...
    setUpAll(() async {
      ...
      aliceDatabase = await databaseFactoryMemory.openDatabase('alice_perf.db');
      bobDatabase = await databaseFactoryMemory.openDatabase('bob_perf.db');
    ...
    test('X3DH Key Exchange Performance', () async {
      ...
      for (int i = 0; i < iterations; i++) {
        ...
        final aliceSessionManager = SignalSessionManager(
          database: await databaseFactoryMemory.openDatabase('alice_x3dh_$i.db'),
          ffiBindings: aliceFFI,
          keyManager: aliceKeyManager,
        );
      ...
      // ignore: unused_local_variable - May be used in callback or assertion
        final bobSessionManager = SignalSessionManager(
          database: await databaseFactoryMemory.openDatabase('bob_x3dh_$i.db'),
          ffiBindings: bobFFI,
          keyManager: bobKeyManager,
        );
