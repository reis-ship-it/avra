...
void main() {
  group('Signal Protocol Security Validation', () {
    ...
    setUpAll(() async {
      ...
      aliceDatabase = await databaseFactoryMemory.openDatabase('alice_sec.db');
      bobDatabase = await databaseFactoryMemory.openDatabase('bob_sec.db');
    ...
    test('Forward Secrecy: Old messages cannot be decrypted with new keys', () async {
      if (!librariesAvailable) {
      ...
      expect(true, isTrue, reason: 'Forward secrecy is provided by Double Ratchet');
    });
    ...
    test('Post-Compromise Security: Recovery after key compromise', () async {
      ...
      final message1 = Uint8List.fromList('Message before compromise'.codeUnits);
      final encrypted1 = await aliceProtocol.encryptMessage(
        plaintext: message1,
      ...
      // through the Double Ratchet algorithm
      expect(true, isTrue, reason: 'Post-compromise security is provided by Double Ratchet');
    });
    ...
    test('Key Isolation: Keys do not leak between sessions', () async {
      ...
      final charlieDatabase = await databaseFactoryMemory.openDatabase('charlie_sec.db');
      final charlieSessionManager = SignalSessionManager(
        database: charlieDatabase,
        ffiBindings: charlieFFI,
      ...
      final charliePreKeyBundle = await charlieKeyManager.generatePreKeyBundle();
      aliceKeyManager.setTestPreKeyBundle('charlie-agent-iso', charliePreKeyBundle);
      
      // Encrypt messages to different recipients
      final messageToBob = Uint8List.fromList('Message to Bob'.codeUnits);
      final messageToCharlie = Uint8List.fromList('Message to Charlie'.codeUnits);
      ...
      try {
        await bobProtocol.decryptMessage(
          encrypted: SignalEncryptedMessage.fromBytes(encryptedToCharlie.toBytes()),
