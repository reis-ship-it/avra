...
async function buildQueryFingerprint(
  ...
  geoBucketType: 'city_code' | 'geohash3',
  profile: {
...
serve(async (req) => {
  ...
  try {
    ...
    // Geo bucket: allow either geohash3 or city_code (default city_code).
    const geoBucketType = (parsed.geo_bucket_type ?? 'city_code') as 'city_code' | 'geohash3'
    if (geoBucketType !== 'geohash3' && geoBucketType !== 'city_code') {
      return new Response(
        JSON.stringify({
          success: false,
          error: { code: 'INVALID_REQUEST', message: 'geo_bucket_type must be "city_code" or "geohash3"' },
        }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      )
    }
