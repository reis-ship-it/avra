...
void main() {
  ...
  const runNative = bool.fromEnvironment('RUN_SIGNAL_NATIVE_TESTS', defaultValue: false);
  ...
  const password = String.fromEnvironment('SIGNAL_SMOKE_PASSWORD', defaultValue: '');
  const communityId =
  ...
  testWidgets('Signal two-device DM (real transport) smoke', (tester) async {
    ...
    await tester.runAsync(() async {
      ...
      final rendezvous =
          client.channel('signal_smoke_rendezvous:$runId');
      ...
      rendezvous.onBroadcast(
        ...
        callback: (payload, {String? event, String? type, String? timestamp}) {
          try {
            final p = (payload as Map?)?.cast<String, dynamic>() ?? const <String, dynamic>{};
      ...
      final peerUserId = await peerUserIdCompleter.future.timeout(
        ...
        onTimeout: () {
          throw TimeoutException('Timed out waiting for peer rendezvous (hello)');
        },
      );
      // ignore: avoid_print
      print('✅ [signal_2dev:$role] Peer discovered userId=$peerUserId');

      // 3) Initialize Signal stack (with real keyserver)
      // ignore: avoid_print
      print('🔎 [signal_2dev:$role] Initializing Signal stack...');
      final Database db =
          await databaseFactoryMemory.openDatabase('signal_two_device_$role.db');
      ...
      if (communityId.isNotEmpty) {
        // ignore: avoid_print
        print('🔎 [signal_2dev:$role] Running community stream smoke for communityId=$communityId');
        ...
        if (role == 'A') {
          ...
          for (var i = 0; i < senderKey32.length; i++) {
            senderKey32[i] = (i * 37 + runId.codeUnitAt(i % runId.length)) & 0xff;
          }
          ...
          final encryptedPayloadBase64 = Aes256GcmFixedKeyCodec.encryptStringToBase64(
            key32: senderKey32,
          ...
          final blob = await communityBlob.future.timeout(const Duration(seconds: 120));
