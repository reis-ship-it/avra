...
/// SPOTS Complete Onboarding Flow Integration Test
///
  ...
  group('Complete Onboarding Flow Integration Tests', () {
    ...
    testWidgets('validates required fields before proceeding',
      ...
      expect(button.onPressed, isNull,
          reason: 'Next button should be disabled when birthday is not selected');
...
Future<void> _testWelcomeStep(WidgetTester tester) async {
  ...
  await tester.pump(const Duration(milliseconds: 600)); // fade out + page change
...
Future<void> _testFavoritePlaces(WidgetTester tester) async {
  ...
  final nyAreaTitle =
      find.descendant(of: nyAreaTile.first, matching: find.text('New York Area'));
  ...
  final parkSlope = find.descendant(of: pageScope, matching: find.text('Park Slope'));
  ...
  // Proceed to next step
  await _swipeToNextStep(tester, expectAfter: find.byType(PreferenceSurveyPage));
}
...
Future<void> _completeOnboarding(WidgetTester tester) async {
  ...
  expect(button.onPressed, isNotNull, reason: 'Complete Setup CTA is disabled.');
  expect(
...
Future<_PumpAnyResult> _pumpUntilAnyFound(
  ...
  throw TestFailure('Timed out waiting for onboarding completion (ai-loading or home).');
}
...
Future<void> _navigateToNextStep(
  ...
  expect(step, findsWidgets, reason: 'Expected current step to be hit-testable');

  final onboarding = find.ancestor(of: step.first, matching: find.byType(OnboardingPage));
