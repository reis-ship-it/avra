...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onSignInRequested(
    ...
    emit(AuthLoading());
    try {
      developer.log('🔐 AuthBloc: Attempting sign in for ${event.email}');
      final user = await signInUseCase(event.email, event.password);
      developer.log('🔐 AuthBloc: Sign in result - user: ${user?.email ?? 'null'}');
      if (user != null) {
        final isOffline = user.isOnline == false;
        developer.log('🔐 AuthBloc: User authenticated successfully');
        emit(Authenticated(user: user, isOffline: isOffline));
      } else {
        developer.log('🔐 AuthBloc: User authentication failed - null user');
        emit(AuthError('Invalid credentials'));
      }
    } catch (e) {
      developer.log('🔐 AuthBloc: Sign in error: $e');
