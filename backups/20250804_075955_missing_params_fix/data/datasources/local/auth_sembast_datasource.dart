  ...
  Future<User?> signIn(String email, String password) async {
    try {
      ...
      if (records.isNotEmpty) {
        ...
        developer.log('🔐 AuthSembastDataSource: User data found: $userData');
        ...
        if (email == 'demo@spots.com' || password.isNotEmpty) {
          ...
          developer.log('🔐 AuthSembastDataSource: Demo user signed in successfully: $email');
      ...
      if (email == 'demo@spots.com') {
        developer.log('🔐 AuthSembastDataSource: Creating demo user on-the-fly');
        ...
        developer.log('🔐 AuthSembastDataSource: Demo user saved to database');
        
        // Save current user
        await _preferencesStore.record('currentUser').put(db, {'key': 'currentUser', 'value': demoUser.id});
        developer.log('🔐 AuthSembastDataSource: Demo user created and signed in successfully: $email');
        developer.log('Demo user created and signed in successfully: $email', name: 'AuthSembastDataSource');
        return demoUser;
      }
      
      developer.log('🔐 AuthSembastDataSource: Invalid credentials for: $email');
      developer.log('Invalid credentials for: $email', name: 'AuthSembastDataSource');
      return null;
    } catch (e) {
      developer.log('🔐 AuthSembastDataSource: Error signing in: $e');
