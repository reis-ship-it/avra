...
class OpenStreetMapDataSourceImpl implements OpenStreetMapDataSource {
  ...
  static const Duration _cacheExpiry = Duration(hours: 2); // Longer cache for community data
  static const Duration _rateLimitDelay = Duration(seconds: 1); // Respectful rate limiting
  ...
  Future<List<Spot>> searchPlaces({
    ...
    try {
      ...
      if (_isCacheValid(cacheKey)) {
        developer.log('OSM: Returning cached results for: $query', name: _logName);
      ...
      if (response.statusCode == 200) {
        ...
        developer.log('OSM: Found ${spots.length} places for: $query', name: _logName);
        return spots;
      } else {
        throw OpenStreetMapException('Nominatim search failed: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<List<Spot>> searchNearbyPlaces({
    ...
    try {
      developer.log('OSM: Searching nearby places: $latitude,$longitude', name: _logName);
      ...
      if (response.statusCode == 200) {
        ...
        developer.log('OSM: Found ${spots.length} nearby places', name: _logName);
        return spots;
      } else {
        throw OpenStreetMapException('Overpass search failed: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<Spot?> getPlaceDetails(String osmId) async {
    try {
      ...
      final url = '$_nominatimBaseUrl/lookup?osm_ids=$osmId&format=json&addressdetails=1&extratags=1';
  ...
  Future<List<Spot>> searchAmenities({
    ...
    try {
      ...
      // Check cache first
      final cacheKey = 'amenities_${latitude}_${longitude}_${amenityType}_$radius';
      if (_isCacheValid(cacheKey)) {
        return List<Spot>.from(_cache[cacheKey]);
      }
      ...
      if (response.statusCode == 200) {
        ...
        developer.log('OSM: Found ${spots.length} $amenityType amenities', name: _logName);
        return spots;
      } else {
        throw OpenStreetMapException('Amenity search failed: ${response.statusCode}');
      }
    } catch (e) {
  ...
  String _buildNominatimSearchUrl({
    ...
    var url = '$_nominatimBaseUrl/search?q=${Uri.encodeComponent(query)}&format=json&addressdetails=1&limit=$limit';
  ...
  String _buildOverpassQuery({
    ...
    final amenityFilter = amenity != null ? '["amenity"="$amenity"]' : '["amenity"]';
  ...
  String _buildAddressFromTags(Map<String, dynamic> tags) {
    ...
    return addressParts.isNotEmpty ? addressParts.join(' ') : tags['name']?.toString() ?? '';
  }
