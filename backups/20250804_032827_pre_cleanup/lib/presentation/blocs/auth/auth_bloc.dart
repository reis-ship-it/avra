...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onSignInRequested(
    ...
    emit(AuthLoading());
    try {
      print('🔐 AuthBloc: Attempting sign in for ${event.email}');
      final user = await signInUseCase(event.email, event.password);
      print('🔐 AuthBloc: Sign in result - user: ${user?.email ?? 'null'}');
      if (user != null) {
        final isOffline = user.isOnline == false;
        print('🔐 AuthBloc: User authenticated successfully');
        emit(Authenticated(user: user, isOffline: isOffline));
      } else {
        print('🔐 AuthBloc: User authentication failed - null user');
