...
class IdealStateLearningService {
  ...
  Future<IdealState> calculateIdealStateFromHistory({
    ...
    try {
      ...
      for (var i = 0; i < historicalMatches.length; i++) {
        final timeDiff = tAtomic.serverTime.difference(timestamps[i].serverTime);
      ...
      final avgSuccessScore = successScores.fold(0.0, (sum, s) => sum + s) /
  ...
  QuantumEntityState _createHeuristicEntityState({
    ...
    final defaultCharacteristics = _getDefaultCharacteristics(entityType, category);
  ...
  Map<String, double> _getDefaultPersonalityState(QuantumEntityType entityType) {
  ...
  Future<void> _updateIdealState({
    ...
    final newAverageSuccessScore = (
      (existingIdealState.averageSuccessScore * existingIdealState.matchCount) +
      successScore
    ) / newMatchCount;
  ...
  List<QuantumEntityState> _combineStates({
    ...
    for (var i = 0; i < math.min(state1.length, state2.length); i++) {
      ...
      combined.add(QuantumEntityState(
        ...
        timing: s1.timing,
        tAtomic: s1.tAtomic,
        normalizationFactor: (weight1 * s1.normalizationFactor + weight2 * s2.normalizationFactor).clamp(0.0, 1.0),
      ));
    }
  ...
  List<QuantumEntityState> _calculateWeightedAverageState({
    ...
    for (final entityType in statesByType.keys) {
      ...
      final normalizedWeights = typeWeights.map((w) => w / totalWeight).toList();
    ...
    for (var i = 0; i < states.length; i++) {
      ...
      for (final entry in state.personalityState.entries) {
        combinedPersonalityState[entry.key] =
            (combinedPersonalityState[entry.key] ?? 0.0) + (weight * entry.value);
      }
    }
  ...
  String _getIdealStateKey(String category, Set<QuantumEntityType> entityTypes) {
