...
class QuantumMatchingResult extends ControllerResult {
  ...
  @override
  List<Object?> get props => [success, error, errorCode, metadata, matchingResult];
}
...
class QuantumMatchingController
  ...
  QuantumMatchingController({
    ...
    MeaningfulConnectionMetricsService? meaningfulConnectionMetricsService,
        ...
        _meaningfulConnectionMetricsService = meaningfulConnectionMetricsService;
  ...
  Future<QuantumMatchingResult> execute(MatchingInput input) async {
    try {
      ...
      return QuantumMatchingResult.success(
        matchingResult: matchingResult,
        metadata: {
          'executionTime': DateTime.now().difference(tAtomic.serverTime).inMilliseconds,
        },
      );
    } catch (e, stackTrace) {
      developer.log(
  ...
  ValidationResult validate(MatchingInput input) {
    ...
    if (!input.isValid) {
      errors.add('At least one entity (event, spot, business, etc.) is required');
    }
  ...
  Future<QuantumEntityState> _convertUserToQuantumState(
    ...
    try {
      ...
      final preferencesProfile = await _preferencesProfileService?.getPreferencesProfile(agentId);
      ...
      if (preferencesProfile != null) {
        ...
        timingState = await _locationTimingService.createTimingQuantumStateFromIntuitive(
      ...
      developer.log('Error getting user timing preferences: $e', name: _logName);
    }
  ...
  Future<QuantumEntityState> _convertEventToQuantumState(
    ...
    AtomicTimestamp tAtomic,
      ...
      final hostPersonality = await _personalityLearning.getCurrentPersonality(event.host.id);
      ...
      if (hostPersonality != null) {
        final hostVibe = await _vibeAnalyzer.compileUserVibe(event.host.id, hostPersonality);
    ...
    EntityTimingQuantumState? timingState;
    try {
      timingState = await _locationTimingService.createTimingQuantumStateFromDateTime(
  ...
  Future<QuantumEntityState> _convertBusinessToQuantumState(
    ...
    if (business.location != null) {
      try {
        ...
        developer.log('Error creating business location state: $e', name: _logName);
      }
    }
  ...
  Future<double> _calculatePredictiveMeaningfulScore({
    ...
    try {
      ...
      final eventEntangledState = await _entanglementService.createEntangledState(
        entityStates: quantumStates,
      );
      final quantumFidelity = await _entanglementService.calculateFidelity(
        userEntangledState,
      ...
      final vibeAlignment = entityCount > 0 ? totalVibeSimilarity / entityCount : 0.0;
  ...
  EntityLocationQuantumState? _findEventLocation(List<QuantumEntityState> quantumStates) {
  ...
  EntityTimingQuantumState? _findEventTiming(List<QuantumEntityState> quantumStates) {
    for (final state in quantumStates) {
  ...
  double _calculateInterestAlignment(
    ...
    // Calculate overlap
    final matchingInterests = userInterests.where((interest) => entityCategories.contains(interest)).length;
    final alignment = matchingInterests / math.max(userInterests.length, entityCategories.length);
  ...
  Future<UnifiedLocation?> _parseLocationString(String locationString) async {
    try {
      ...
      developer.log('Error geocoding location string "$locationString": $e', name: _logName);
    }
