...
import 'package:spots_knot/models/knot/fabric_snapshot.dart';
...
class KnotOrchestratorService {
  ...
  Future<PersonalityKnot> getOrGenerateKnot(
    PersonalityProfile profile,
  ) async {
    try {
  ...
  Future<KnotWorldsheet?> createWorldsheetForGroup({
    ...
    try {
      developer.log(
        'Creating worldsheet for group: $groupId',
        name: _logName,
      );
  ...
  Future<void> setupNewUser(PersonalityProfile profile) async {
    try {
      developer.log(
        'Setting up new user: ${profile.agentId}',
        name: _logName,
      );
