...
class KnotString {
  ...
  PersonalityKnot _interpolateKnots(
    ...
    final interpolatedJones = _interpolatePolynomials(jones1, jones2, factor);
    
    final alexander1 = knot1.invariants.alexanderPolynomial;
    final alexander2 = knot2.invariants.alexanderPolynomial;
    final interpolatedAlexander = _interpolatePolynomials(
      alexander1,
      alexander2,
      factor,
    );
    ...
    // Interpolate all invariants
    final interpolatedSignature = ((knot1.invariants.signature * (1 - factor)) +
            (knot2.invariants.signature * factor))
        .round();
    
    final interpolatedBridgeNumber = ((knot1.invariants.bridgeNumber * (1 - factor)) +
            (knot2.invariants.bridgeNumber * factor))
        .round();
    
    final interpolatedBraidIndex = ((knot1.invariants.braidIndex * (1 - factor)) +
            (knot2.invariants.braidIndex * factor))
        .round();
    
    final interpolatedDeterminant = ((knot1.invariants.determinant * (1 - factor)) +
            (knot2.invariants.determinant * factor))
        .round();
    
    // For optional invariants, use first knot's value if both have it, otherwise null
    final interpolatedUnknottingNumber = knot1.invariants.unknottingNumber != null &&
    ...
    final interpolatedArfInvariant = knot1.invariants.arfInvariant != null &&
    ...
    final interpolatedHyperbolicVolume = knot1.invariants.hyperbolicVolume != null &&
            knot2.invariants.hyperbolicVolume != null
        ? (knot1.invariants.hyperbolicVolume! * (1 - factor)) +
            (knot2.invariants.hyperbolicVolume! * factor)
        : null;
    
    // HOMFLY polynomial interpolation (if both have it)
    final interpolatedHomfly = knot1.invariants.homflyPolynomial != null &&
            knot2.invariants.homflyPolynomial != null
        ? _interpolatePolynomials(
  ...
  List<double> _interpolatePolynomials(
    ...
    for (int i = 0; i < maxLength; i++) {
      ...
      interpolated.add(val1 * (1 - factor) + val2 * factor);
    }
  ...
  List<double> _interpolateBraidData(
    ...
    final interpolatedStrands = ((strands1 * (1 - factor)) + (strands2 * factor)).round();
  ...
  /// Extrapolate future knot using evolution dynamics
  /// 
  /// Uses simplified evolution: K(t_future) ≈ K(t_last) + ΔK/Δt · Δt
  PersonalityKnot _extrapolateFutureKnot(
    ...
    // Calculate evolution rate (change per unit time)
    final evolutionRate = timeDelta.inMilliseconds / historyDelta.inMilliseconds;
    
    // Extrapolate invariants
    final lastKnot = last.knot;
    final secondLastKnot = secondLast.knot;
    ...
    final crossingDelta = lastKnot.invariants.crossingNumber -
        secondLastKnot.invariants.crossingNumber;
    final extrapolatedCrossings = (lastKnot.invariants.crossingNumber +
            (crossingDelta * evolutionRate))
        .round()
        .clamp(0, 100);
    
    final writheDelta = lastKnot.invariants.writhe - secondLastKnot.invariants.writhe;
    final extrapolatedWrithe = (lastKnot.invariants.writhe + (writheDelta * evolutionRate))
        .round();
    
    // Extrapolate all other invariants
    final signatureDelta = lastKnot.invariants.signature - secondLastKnot.invariants.signature;
    final extrapolatedSignature = (lastKnot.invariants.signature + (signatureDelta * evolutionRate))
        .round();
    
    final bridgeDelta = lastKnot.invariants.bridgeNumber - secondLastKnot.invariants.bridgeNumber;
    final extrapolatedBridgeNumber = (lastKnot.invariants.bridgeNumber + (bridgeDelta * evolutionRate))
        .round()
        .clamp(1, 100);
    
    final braidIndexDelta = lastKnot.invariants.braidIndex - secondLastKnot.invariants.braidIndex;
    final extrapolatedBraidIndex = (lastKnot.invariants.braidIndex + (braidIndexDelta * evolutionRate))
        .round()
        .clamp(1, 12);
    
    final determinantDelta = lastKnot.invariants.determinant - secondLastKnot.invariants.determinant;
    final extrapolatedDeterminant = (lastKnot.invariants.determinant + (determinantDelta * evolutionRate))
        .round();
    
    // For optional invariants, extrapolate if both have values
    final extrapolatedUnknottingNumber = lastKnot.invariants.unknottingNumber != null &&
            secondLastKnot.invariants.unknottingNumber != null
        ? (lastKnot.invariants.unknottingNumber! +
                ((lastKnot.invariants.unknottingNumber! - secondLastKnot.invariants.unknottingNumber!) *
                    evolutionRate))
            .round()
            .clamp(0, lastKnot.invariants.crossingNumber)
        : null;
    
    final extrapolatedArfInvariant = lastKnot.invariants.arfInvariant != null &&
            secondLastKnot.invariants.arfInvariant != null
        ? (lastKnot.invariants.arfInvariant! +
                ((lastKnot.invariants.arfInvariant! - secondLastKnot.invariants.arfInvariant!) *
                    evolutionRate))
            .round() %
            2
        : null;
    
    final extrapolatedHyperbolicVolume = lastKnot.invariants.hyperbolicVolume != null &&
            secondLastKnot.invariants.hyperbolicVolume != null
        ? (lastKnot.invariants.hyperbolicVolume! +
                ((lastKnot.invariants.hyperbolicVolume! - secondLastKnot.invariants.hyperbolicVolume!) *
                    evolutionRate))
            .clamp(0.0, double.infinity)
        : null;
    
    // HOMFLY polynomial extrapolation (if available)
    final extrapolatedHomfly = lastKnot.invariants.homflyPolynomial != null &&
...
class KnotEvolutionStringService {
  ...
  Future<PersonalityKnot?> getKnotAtTime(
    String agentId,
    DateTime t,
  ) async {
    try {
  ...
  Future<List<PersonalityKnot>> getEvolutionTrajectory({
    ...
    try {
      ...
      return string.getEvolutionTrajectory(
        start: start,
        end: end,
        step: step,
      );
  ...
  Future<EvolutionAnalysis> analyzeEvolutionPatterns(String agentId) async {
    try {
      ...
      return EvolutionAnalysis(
        hasPatterns: cycles.isNotEmpty || trends.isNotEmpty || milestones.isNotEmpty,
        cycles: cycles,
        trends: trends,
  ...
  List<EvolutionCycle> _detectCycles(List<KnotSnapshot> snapshots) {
    ...
    final complexities = snapshots.map((s) {
      return s.knot.invariants.crossingNumber *
          s.knot.invariants.writhe.abs();
    }).toList();
  ...
  List<EvolutionMilestone> _detectMilestones(List<KnotSnapshot> snapshots) {
    ...
    for (int i = 1; i < snapshots.length; i++) {
      ...
      if (jones1.length != jones2.length) {
        milestones.add(EvolutionMilestone(
          type: MilestoneType.knotTypeChange,
          timestamp: current.timestamp,
          description: 'Knot type changed (polynomial length: ${jones1.length} → ${jones2.length})',
        ));
      } else {
      ...
      final complexity1 = previous.knot.invariants.crossingNumber *
          previous.knot.invariants.writhe.abs();
      final complexity2 = current.knot.invariants.crossingNumber *
          current.knot.invariants.writhe.abs();
      
      final complexityChange = ((complexity2 - complexity1) / complexity1.clamp(1, 100))
          .abs();
      ...
      if (complexityChange > 0.3) {
        milestones.add(EvolutionMilestone(
          ...
          timestamp: current.timestamp,
          description: 'Complexity change: ${(complexityChange * 100).toStringAsFixed(1)}%',
        ));
      }
    }
