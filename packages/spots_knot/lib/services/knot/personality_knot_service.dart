...
class PersonalityKnotService {
  ...
  final QuantumVibeEngine? _quantumEngine;
  bool _initialized = false;
  
  PersonalityKnotService({
    QuantumVibeEngine? quantumEngine,
    ...
    try {
      ...
      try {
        ...
        if (e.toString().contains('Should not initialize flutter_rust_bridge twice')) {
          developer.log('Rust library already initialized (test mode?)', name: _logName);
          _initialized = true;
          return;
        }
        rethrow;
  ...
  Future<PersonalityKnot> generateKnot(PersonalityProfile profile) async {
    developer.log('Generating knot for agentId: ${profile.agentId}', name: _logName);
  ...
  Future<double> calculateCompatibility(
    ...
    developer.log(
      'Calculating compatibility between knots',
      name: _logName,
    );
    ...
    try {
      ...
      developer.log(
        '✅ Compatibility calculated: $compat',
        name: _logName,
      );
