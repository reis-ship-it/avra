...
class QuantumOutcomeLearningService {
  ...
  Future<QuantumLearningData> learnFromOutcome({
    ...
    try {
      ...
      final successMetrics = await _successAnalysisService.analyzeEventSuccess(eventId);
  ...
  Future<double> _calculateQuantumSuccessScore({
    ...
    if (successMetrics.revenueVsProjected > 0) {
      ...
      financialPerformance = (1.0 + successMetrics.revenueVsProjected).clamp(0.0, 1.0);
    } else {
      // If revenue is below projected, score decreases
      financialPerformance = (0.5 + successMetrics.revenueVsProjected).clamp(0.0, 1.0);
    }
    ...
    // Weighted average
    final successScore = (
      _attendanceWeight * attendanceRate +
  ...
  Future<void> _updateIdealState({
    ...
    final timeDiff = tAtomic.serverTime.difference(existingIdealState.createdAt.serverTime);
    final timeDiffDays = timeDiff.inDays.toDouble();
