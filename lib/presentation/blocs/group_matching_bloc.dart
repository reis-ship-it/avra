...
import 'package:avrai/core/models/group_matching_input.dart';
...
class GroupMatchingBloc extends Bloc<GroupMatchingEvent, GroupMatchingState> {
  ...
  Future<void> _onStartGroupFormation(
    ...
    try {
      ...
      final friendsFuture = _formationService.getFriendsList(event.currentUserId);
  ...
  Future<void> _onLoadFriendsList(
    ...
    try {
      ...
      _friendAgentIds = await _formationService.getFriendsList(event.currentUserId);
  ...
  Future<void> _onFormGroup(
    ...
    try {
      if (_selectedMemberAgentIds.isEmpty && event.nearbyUsers == null && event.friendAgentIds == null) {
        emit(GroupMatchingError(
      ...
      } else if (event.friendAgentIds != null && event.friendAgentIds!.isNotEmpty) {
  ...
  Future<void> _onStartGroupSearch(
    ...
    try {
      ...
      for (final matchedSpot in result.matchingResult!.matchedSpots) {
        compatibilityScores[matchedSpot.spot.id] = matchedSpot.groupCompatibility;
      }
