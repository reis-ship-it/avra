...
class _ReceiptDetailPageState extends State<ReceiptDetailPage> {
  ...
  Future<void> _sign() async {
    ...
    try {
      ...
      developer.log('sign_existing failed', name: 'ReceiptDetailPage', error: e, stackTrace: st);
      if (!mounted) return;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Op: ${receipt.event.op.wireName} • Revision: ${receipt.event.revision}'),
            Text('Occurred: ${receipt.event.occurredAt.toLocal()}'),
            if (receipt.event.entityType != null || receipt.event.entityId != null)
              Text('Entity: ${receipt.event.entityType ?? ''}:${receipt.event.entityId ?? ''}'),
            if (receipt.event.category != null) Text('Category: ${receipt.event.category}'),
            if (receipt.event.cityCode != null || receipt.event.localityCode != null)
              Text('Geo: ${receipt.event.cityCode ?? '-'} / ${receipt.event.localityCode ?? '-'}'),
          ],
        ),
      ),
    );
  ...
  Widget _payloadCard(LedgerReceiptV0 receipt) {
    final payloadPretty = const JsonEncoder.withIndent('  ').convert(receipt.event.payload);
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
