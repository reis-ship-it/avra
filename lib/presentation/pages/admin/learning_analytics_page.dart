...
class _LearningAnalyticsPageState extends State<LearningAnalyticsPage> {
  ...
  final SupabaseService? _supabaseService = GetIt.instance.isRegistered<SupabaseService>()
  ...
  Future<void> _loadLearningData() async {
    ...
    try {
      ...
      if (userId == null) {
        developer.log('No user ID available for learning analytics', name: _logName);
        setState(() {
          _isLoading = false;
        });
        return;
  ...
  Widget _buildSummaryCards() {
    ...
    for (final event in _learningHistory) {
      ...
      improvementsByDimension.putIfAbsent(event.dimension, () => []).add(event.improvement);
      dataSourceCounts[event.dataSource] = (dataSourceCounts[event.dataSource] ?? 0) + 1;
    }
    
    final avgImprovements = <String, double>{};
    for (final entry in improvementsByDimension.entries) {
      final avg = entry.value.fold(0.0, (sum, imp) => sum + imp) / entry.value.length;
  ...
  Widget _buildLearningHistoryList() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        ..._learningHistory.take(50).map((event) => _buildLearningEventCard(event)),
      ],
    );
  }
