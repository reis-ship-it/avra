...
class ModelRetrainingService {
  ...
  Map<String, dynamic> _parseTrainingMetrics(String output) {
    ...
    if (testLossMatch != null) {
      metrics['test_loss'] = double.tryParse(testLossMatch.group(1) ?? '0') ?? 0.0;
    }
    
    // Parse validation loss
    final valLossMatch = RegExp(r'Val Loss:\s*([\d.]+)').firstMatch(output);
    if (valLossMatch != null) {
      metrics['val_loss'] = double.tryParse(valLossMatch.group(1) ?? '0') ?? 0.0;
    }
    
    // Parse training loss
    final trainLossMatch = RegExp(r'Train Loss:\s*([\d.]+)').firstMatch(output);
    if (trainLossMatch != null) {
      metrics['train_loss'] = double.tryParse(trainLossMatch.group(1) ?? '0') ?? 0.0;
    }
    
    // Parse early stopping epoch
    final earlyStopMatch = RegExp(r'Early stopping at epoch\s*(\d+)').firstMatch(output);
    if (earlyStopMatch != null) {
      metrics['early_stopping_epoch'] = int.tryParse(earlyStopMatch.group(1) ?? '0') ?? 0;
    }
  ...
  Future<bool> validateModel(String modelPath) async {
    try {
      ...
      if (!modelPath.endsWith('.onnx')) {
        developer.log('Model file is not ONNX format: $fullPath', name: _logName);
        return false;
      }
      
      developer.log('✅ Model validated: $modelPath (${(size / 1024).toStringAsFixed(1)} KB)', name: _logName);
      return true;
    } catch (e, stackTrace) {
      developer.log(
