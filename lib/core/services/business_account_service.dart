...
class BusinessAccountService {
  ...
  Future<BusinessAccount> createBusinessAccount({
    ...
    try {
      ...
      _logger.info('Created business account: ${businessAccount.id}', tag: _logName);
  ...
  Future<List<BusinessAccount>> getBusinessAccountsByUser(String userId) async {
    try {
      ...
      _logger.error('Error getting business accounts by user', error: e, tag: _logName);
      return [];
    }
  }
  ...
  Future<BusinessAccount> addExpertConnection(
    ...
    try {
      ...
      final updated = account.copyWith(
        ...
        pendingConnectionIds: account.pendingConnectionIds.where((id) => id != expertId).toList(),
        updatedAt: DateTime.now(),
      );
  ...
  Future<BusinessAccount> requestExpertConnection(
    ...
    try {
      ...
      _logger.error('Error requesting expert connection', error: e, tag: _logName);
      rethrow;
    }
  }
