...
class GeoLocalityPackService {
  ...
  Future<_PackCache?> _loadPack(String cityCode) async {
    if (_cacheByCityCode.containsKey(cityCode)) return _cacheByCityCode[cityCode];
      ...
      if (await indexFile.exists()) {
        ...
        if (decoded is Map) {
          ...
          for (final entry in decoded.entries) {
            ...
            final v = entry.value;
            if (v is List) {
              m[k] = v.map((e) => e.toString()).where((s) => s.isNotEmpty).toList();
            }
          }
          index = m;
        }
  ...
  Future<({String localityCode, String cityCode, String displayName})?>
    ...
    final candidates = candidateSet.isEmpty ? const <String>[] : candidateSet.toList();
  ...
  GeoPolygonV1 _parseGeoJsonToPolygonPreferLargest(Map<String, dynamic> geojson) {
