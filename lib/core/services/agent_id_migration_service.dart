...
class AgentIdMigrationService {
  ...
  Future<MigrationResult> migrateAllMappings({
    ...
    try {
      ...
      developer.log('📊 Found $totalCount plaintext mappings to migrate', name: _logName);
      ...
      for (int batch = 0; batch < batches; batch++) {
        ...
        for (final mapping in plaintextMappings) {
          try {
            ...
            if (alreadyMigrated) {
              skipped++;
              developer.log('⏭️  Skipping already migrated: $userId', name: _logName);
              continue;
            }
            
            if (dryRun) {
              migrated++;
              developer.log('🔍 [DRY RUN] Would migrate: $userId', name: _logName);
              continue;
            }
  ...
  Future<VerificationResult> verifyMigration() async {
    try {
      ...
      // Find discrepancies
      final notMigrated = plaintextUserIds
          .where((id) => !secureUserIds.contains(id))
          .toList();
      
      final extraSecure = secureUserIds
          .where((id) => !plaintextUserIds.contains(id))
          .toList();
      
      // Verify decryption works for sample
      int decryptionErrors = 0;
      if (secureUserIds.isNotEmpty) {
        final sampleSize = secureUserIds.length > 10 ? 10 : secureUserIds.length;
