...
class DisputeEvidenceStorageService {
  ...
  Future<String> uploadEvidenceImage({
    ...
    if (!_supabaseService.isAvailable) {
      throw StateError('Supabase not available (cannot upload dispute evidence)');
    }
  ...
  bool _isSupabaseObjectRef(String url) => url.startsWith('$_supabaseScheme://');
  ...
  ({String bucket, String objectPath}) _parseSupabaseRef(String url) {
    ...
    final objectPath = uri.path.startsWith('/') ? uri.path.substring(1) : uri.path;
  ...
  String _normalizeExtension(String? fileExtension, {required String contentType}) {
