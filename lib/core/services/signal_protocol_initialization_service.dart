...
class SignalProtocolInitializationService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      developer.log('Initializing Signal Protocol FFI bindings (base library)...', name: _logName);
      await _signalProtocol.initialize();
      developer.log('✅ Signal Protocol FFI bindings initialized', name: _logName);
      ...
      _initialized = true;
      developer.log('✅ Signal Protocol initialized successfully', name: _logName);
      ...
      if (!_skipProductionVerification) {
        ...
        developer.log('Skipping production verification (test mode)', name: _logName);
      }
    } catch (e, stackTrace) {
      developer.log(
  ...
  Future<void> _verifyProductionReadiness() async {
    try {
      developer.log('Verifying Signal Protocol production readiness...', name: _logName);
      ...
      try {
        ...
        if (e.toString().contains('session') || e.toString().contains('prekey')) {
