...
class BusinessVerificationService {
  ...
  }) : _businessAccountService = businessAccountService ?? BusinessAccountService();
  ...
  Future<BusinessVerification> submitVerification({
    ...
    try {
      _logger.info('Submitting verification for business: ${business.id}', tag: _logName);
  ...
  Future<BusinessVerification> verifyAutomatically(BusinessAccount business) async {
    try {
      _logger.info('Attempting automatic verification for: ${business.id}', tag: _logName);
      ...
      if (business.website != null && business.website!.isNotEmpty) {
        ...
        if (isValidWebsite) {
          final verification = BusinessVerification(
            ...
            websiteVerificationUrl: business.website, // In production, would verify ownership
      ...
      throw Exception('Automatic verification failed. Please submit documents for manual review.');
    } catch (e) {
  ...
  Future<BusinessVerification> approveVerification(
    ...
    try {
      ...
      final business = await _getBusinessAccount(verification.businessAccountId);
      if (business != null) {
  ...
  Future<BusinessVerification?> getVerification(String businessAccountId) async {
    try {
  ...
  Future<void> _updateBusinessVerification(
    ...
    try {
      ...
      developer.log('Updated business verification: ${business.id}', name: _logName);
    } catch (e) {
