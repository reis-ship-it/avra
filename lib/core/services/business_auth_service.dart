...
class BusinessAuthService {
  ...
  Future<BusinessAuthResult> authenticate({
    ...
    try {
      ...
      developer.log('Business authenticated: $username (business: ${verifyResult.businessId})', name: _logName);
  ...
  Future<_VerifyResult> _verifyCredentials(String username, String password, String? twoFactorCode) async {
    try {
      ...
      if (response.status == 200 && data?['success'] == true) {
        developer.log('Business credentials verified successfully', name: _logName);
        return _VerifyResult(
          success: true,
      ...
      developer.log('Business auth failed: $error (status: ${response.status})', name: _logName);
