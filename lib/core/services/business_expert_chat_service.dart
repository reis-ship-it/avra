...
class BusinessExpertChatService {
  ...
  Future<BusinessExpertMessage> sendMessage({
    ...
    try {
      ...
      if (encrypt) {
        final recipientId = senderType == MessageSenderType.business
            ? expertId
            : businessId;
        final encrypted = await _encryptionService.encrypt(content, recipientId);
  ...
  Future<List<BusinessExpertMessage>> getMessageHistory(
    ...
    try {
      ...
      final messages = (response as List)
          .map((json) => BusinessExpertMessage.fromJson(
  ...
  Future<int> getUnreadCount(String businessIdOrExpertId, bool isBusiness) async {
    try {
      ...
      try {
        ...
        if (isBusiness) {
          query = query.eq('recipient_type', 'business').eq('recipient_id', businessIdOrExpertId);
        } else {
          query = query.eq('recipient_type', 'expert').eq('recipient_id', businessIdOrExpertId);
        }
  ...
  Stream<BusinessExpertMessage> subscribeToMessages(String conversationId) {
    ...
    try {
      final client = _supabaseService.client;
      final channel = client.channel('business_expert_messages:$conversationId');
