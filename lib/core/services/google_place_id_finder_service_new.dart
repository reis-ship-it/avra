...
/// Finds Google Place IDs for community spots using Places API (New)
/// OUR_GUTS.md: "Community, Not Just Places" - Link community spots to Google Maps
class GooglePlaceIdFinderServiceNew {
  static const String _logName = 'GooglePlaceIdFinderServiceNew';
  ...
  Future<String?> findPlaceId(Spot spot) async {
    try {
      developer.log('Finding Google Place ID (New API) for: ${spot.name}', name: _logName);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (final place in places) {
          ...
          final placeName = displayName?['text'] as String? ?? '';
          final placeId = place['id'] as String? ?? '';
          final location = place['location'] as Map<String, dynamic>?;
          
          if (location == null || placeId.isEmpty) continue;
          
          final placeLat = (location['latitude'] ?? 0.0).toDouble();
          final placeLng = (location['longitude'] ?? 0.0).toDouble();
          ...
          if (similarity >= _nameSimilarityThreshold) {
            ...
            return placeId.replaceFirst('places/', '');
          }
        }
      }
  ...
  Future<String?> _findPlaceIdByTextSearch(Spot spot) async {
    try {
      ...
      final requestBody = {
        ...
        'locationBias': {
          'circle': {
            ...
            'radius': (_maxDistanceMeters * 2).toDouble(), // More lenient for text search
          },
        },
      };
      ...
      if (response.statusCode == 200) {
        ...
        final displayName = firstPlace['displayName'] as Map<String, dynamic>?;
        final placeName = displayName?['text'] as String? ?? '';
        final placeId = firstPlace['id'] as String? ?? '';
        final location = firstPlace['location'] as Map<String, dynamic>?;
