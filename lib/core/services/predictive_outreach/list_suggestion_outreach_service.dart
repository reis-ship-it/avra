...
class ListSuggestionOutreachService {
          ...
          if (outreachScore >= 0.7) {
            await _ai2aiCommunication.sendOutreachMessage(
              ...
              payload: {
                ...
                'reasoning': recommendation.reasoning ?? 'High compatibility list suggestion',
              },
            );
          }
        } catch (e) {
          developer.log('Error processing list recommendation: $e', name: _logName);
          continue;
        }
      }
    } catch (e, stackTrace) {
  ...
  Future<void> processExpertCuratedListSuggestions({
    ...
    try {
      ...
      final expertLists = await _expertRecommendationsService.getExpertCuratedLists(
        user,
        maxResults: 20,
      );
      ...
      for (final expertList in expertLists) {
        try {
          final curatorProfile = await _personalityLearning.getCurrentPersonality(expertList.curator.id);
          ...
          final outreachScore = _combineListOutreachSignals(
            stringPrediction: stringPred.predictedCompatibility,
            quantumTrajectory: quantumTraj.peakCompatibility,
            currentCompatibility: expertList.respectCount / 100.0, // Normalize respect count
          );
          ...
          if (outreachScore >= 0.7) {
            await _ai2aiCommunication.sendOutreachMessage(
              ...
              payload: {
                ...
                'curator_name': expertList.curator.displayName ?? expertList.curator.id,
                'compatibility_score': outreachScore,
  ...
  Future<List<ListRecommendation>> _getListRecommendations(UnifiedUser user) async {
