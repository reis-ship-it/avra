...
class OutreachQueueProcessor {
    ...
    try {
      ...
      for (final message in messages) {
        try {
          ...
          await _updateMessageStatus(message.id, OutreachQueueStatus.processing);
  ...
  OutreachQueueMessage parseQueueMessage(Map<String, dynamic> row) {
    return OutreachQueueMessage(
      ...
      stringPredictionScore: (row['string_prediction_score'] as num?)?.toDouble(),
      quantumTrajectoryScore: (row['quantum_trajectory_score'] as num?)?.toDouble(),
