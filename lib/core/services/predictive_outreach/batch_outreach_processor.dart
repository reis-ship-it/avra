...
class BatchOutreachProcessor {
  ...
  Future<TypeProcessingResult> _processCommunities() async {
    try {
      final communityIds = await _getActiveCommunityIds(limit: _communityBatchSize);
      ...
      for (final communityId in communityIds) {
        try {
          await _communityOutreach.processCommunityOutreach(communityId: communityId);
          success++;
        } catch (e) {
          developer.log('Error processing community $communityId: $e', name: _logName);
          failures++;
        }
        processed++;
      }
  ...
  Future<TypeProcessingResult> _processBusinesses() async {
    try {
      final businessIds = await _getActiveBusinessIds(limit: _businessBatchSize);
      ...
      for (final businessId in businessIds) {
        try {
          await _businessOutreach.processBusinessExpertOutreach(businessId: businessId);
          await _businessOutreach.processBusinessBusinessOutreach(businessId: businessId);
          success++;
        } catch (e) {
          developer.log('Error processing business $businessId: $e', name: _logName);
          failures++;
        }
        processed++;
      }
  ...
  Future<TypeProcessingResult> _processExperts() async {
    try {
      ...
      for (final expertId in expertIds) {
        try {
          await _expertOutreach.processExpertLearningOutreach(expertId: expertId);
          await _expertOutreach.processExpertBusinessOutreach(expertId: expertId);
          success++;
        } catch (e) {
          developer.log('Error processing expert $expertId: $e', name: _logName);
          failures++;
        }
        processed++;
      }
    ...
    try {
      ...
      for (final userId in userIds) {
        try {
          ...
          await _listOutreach.processExpertCuratedListSuggestions(userId: userId);
          success++;
        } catch (e) {
          developer.log('Error processing lists for user $userId: $e', name: _logName);
          failures++;
        }
        processed++;
      }
