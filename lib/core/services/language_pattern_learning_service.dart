...
class LanguagePatternLearningService {
  ...
  Future<LanguageProfile?> getLanguageProfile(String userId) async {
    try {
      ...
      final store = _getLanguageProfileStore();
  ...
  Future<String> getLanguageStyleSummary(String userId) async {
    ...
    if (profile.averageSentenceLength > 0) {
      buffer.writeln('- Average sentence length: ${profile.averageSentenceLength.toStringAsFixed(1)} words');
    }
    
    // Tone
    buffer.writeln('- Formality: ${_formatTone(profile.tone['formality'] ?? 0.5)}');
    buffer.writeln('- Enthusiasm: ${_formatTone(profile.tone['enthusiasm'] ?? 0.5)}');
    buffer.writeln('- Directness: ${_formatTone(profile.tone['directness'] ?? 0.5)}');
  ...
  Map<String, double> _extractVocabulary(List<String> words) {
    ...
    final maxFreq = wordFreq.values.isEmpty ? 1 : wordFreq.values.reduce(math.max);
  ...
  Map<String, dynamic> _analyzeSentenceStructure(List<String> sentences) {
    ...
    final totalWords = sentences.fold<int>(
      0,
      (sum, sentence) => sum + sentence.split(RegExp(r'\s+')).where((w) => w.isNotEmpty).length,
    );
    ...
    final maxPunctFreq = punctFreq.values.isEmpty ? 1 : punctFreq.values.reduce(math.max);
    final punctUsage = punctFreq.map((punct, freq) => MapEntry(punct, freq / maxPunctFreq));
  ...
  Map<String, double> _analyzeTone(String message, List<String> words) {
    ...
    final formalWords = {'please', 'thank', 'appreciate', 'regarding', 'furthermore', 'however'};
    final casualWords = {'yeah', 'yep', 'nah', 'gonna', 'wanna', 'gotta', 'cool', 'awesome'};
    ...
    final enthusiasticWords = {'awesome', 'amazing', 'love', 'great', 'fantastic', 'excited'};
    ...
    final indirectWords = {'maybe', 'perhaps', 'might', 'could', 'possibly', 'sort of', 'kind of'};
  ...
  LanguageProfile _updateProfileWithAnalysis(
    ...
    final confidence = math.min(1.0, newMessageCount / 100.0); // Max confidence at 100 messages
  ...
  Future<void> _saveLanguageProfile(String agentId, LanguageProfile profile) async {
    try {
      final db = await SembastDatabase.database;
      final store = _getLanguageProfileStore();
      await store.record('$_dataKeyPrefix$agentId').put(db, profile.toJson());
    } catch (e, stackTrace) {
      developer.log(
  ...
  /// Get language profile store
  StoreRef<String, Map<String, dynamic>> _getLanguageProfileStore() {
    return stringMapStoreFactory.store('language_profiles');
  }
