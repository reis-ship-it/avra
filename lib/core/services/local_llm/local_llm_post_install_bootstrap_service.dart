...
class LocalLlmPostInstallBootstrapService {
  ...
  })  : _agentIdService = agentIdService ?? (_tryGet<AgentIdService>() ?? AgentIdService()),
        _packs = packs ?? LocalLlmModelPackManager(),
        _onboardingData =
            onboardingDataService ?? (_tryGet<OnboardingDataService>() ?? OnboardingDataService()),
        _suggestionEvents =
            suggestionEventStore ??
                (_tryGet<OnboardingSuggestionEventStore>() ??
                    OnboardingSuggestionEventStore());

  Future<Database> _db() async => SembastDatabase.database;

  StoreRef<String, Map<String, dynamic>> get _store =>
      SembastDatabase.onboardingStore;

  Future<LocalLlmBootstrapState?> getBootstrapStateForUser(String userId) async {
    try {
      final agentId = await _agentIdService.getUserAgentId(userId);
      final db = await _db();
      final record = await _store.record('$_bootstrapKeyPrefix$agentId').get(db);
  ...
  Future<LocalLlmBootstrapState?> maybeBootstrapForUser(String userId) async {
    try {
      ...
      final db = await _db();
      await _store.record('$_bootstrapKeyPrefix$agentId').put(db, state.toJson());

      // Store refinement prompt definitions so UI can show later.
      if (refinements.isNotEmpty) {
        await _store.record('$_refinementKeyPrefix$agentId').put(db, <String, dynamic>{
          'updatedAtMs': now,
  ...
  Future<List<LocalLlmRefinementPrompt>> getRefinementPromptsForUser(
    ...
    try {
      ...
      final db = await _db();
      final record =
          await _store.record('$_refinementKeyPrefix$agentId').get(db);
  ...
  Future<List<LocalLlmRefinementPrompt>> getPendingRefinementPromptsForUser(
    ...
    if (state == null || state.pendingRefinementPromptIds.isEmpty) return const [];
  ...
  String _renderSystemPromptFromMemory(
    ...
    buf.writeln('- Prefer authentic local spots and community-forward suggestions.');
