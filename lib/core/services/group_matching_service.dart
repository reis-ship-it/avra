...
import 'package:avrai/core/models/group_matching_result.dart';
import 'package:avrai/core/models/group_session.dart';
import 'package:avrai_core/models/unified_location_data.dart';
import 'package:avrai/core/models/unified_models.dart';
import 'package:avrai/core/ai/quantum/location_compatibility_calculator.dart' as location_calc;
import 'package:avrai_quantum/services/quantum/location_timing_quantum_state_service.dart' as timing_calc;
...
class GroupMatchingService {
  ...
  Future<GroupMatchingResult> matchGroupAgainstSpots({
    ...
    try {
      ...
      final groupEntangledState = await _entanglementService.createEntangledState(
      ...
      matchedSpots.sort((a, b) =>
          b.groupCompatibility.compareTo(a.groupCompatibility));
  ...
  Future<GroupMatchedSpot?> _calculateGroupSpotCompatibility({
    ...
    try {
      ...
      double knotCompatibility = 0.5; // Default: neutral (spots don't have knots)
        ...
        for (final agentId in memberKnots.keys) {
          try {
            ...
            final futureKnot = await _stringService.predictFutureKnot(
              agentId,
              DateTime.now().add(const Duration(hours: 2)), // Assume visit in 2 hours
            );
            if (futureKnot != null) {
        ...
        for (final memberState in memberQuantumStates) {
          if (memberState.location != null) {
            ...
            final memberLocation = UnifiedLocation(
              latitude: memberState.location!.latitudeQuantumState * 180.0 - 90.0, // Denormalize
              longitude: memberState.location!.longitudeQuantumState * 360.0 - 180.0, // Denormalize
            );
  ...
  Future<double> _calculateQuantumCompatibility({
    ...
    try {
      ...
      final spotEntangledState = await _entanglementService.createEntangledState(
  ...
  Future<double> _calculateMemberSpotCompatibility({
    ...
    try {
      final memberEntangledState = await _entanglementService.createEntangledState(
  ...
  Future<QuantumEntityState> _createSpotQuantumState(
    ...
    final spotVibe = SpotVibe.fromSpotCharacteristics(
      ...
      final locationState = await _locationTimingService.createLocationQuantumState(
