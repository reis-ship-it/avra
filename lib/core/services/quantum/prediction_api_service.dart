...
class PredictionAPIService {
      ...
      if (attendeeUsers.isNotEmpty) {
        ...
        for (final attendeeUser in attendeeUsers) {
          try {
            ...
            final agentId = await _agentIdService.getUserAgentId(attendeeUser.id);
  ...
  Future<VibeEvolutionPredictions> getVibeEvolutionPredictions({
    ...
    try {
      ...
      final userVibe = await _vibeAnalyzer.compileUserVibe(userId, currentPersonality);
      ...
      for (final event in upcomingEvents.take(maxEvents)) {
        try {
          ...
          final prediction = await _hypotheticalService.predictUserEventCompatibility(
            userId: userId,
            eventId: event.id,
          );
          ...
          for (final key in predictedVibe.keys) {
            ...
            predictedVibe[key] = (predictedVibe[key]! * (1 - evolutionFactor) +
                    evolutionFactor)
                .clamp(0.0, 1.0);
          }
          ...
          if (_stringService != null) {
            try {
              final agentIdForUser = await _agentIdService.getUserAgentId(userId);
    ...
    try {
      ...
      for (final event in upcomingEvents.take(maxEvents)) {
        try {
          ...
          final predictedPostEventState = Map<String, double>.from(currentJourneyState);
          
          // Apply journey evolution based on prediction score
          final evolutionFactor = prediction.predictionScore * 0.1; // Max 10% change
          for (final key in predictedPostEventState.keys) {
            predictedPostEventState[key] = (predictedPostEventState[key]! *
                    (1 + evolutionFactor))
                .clamp(0.0, 1.0);
          }

          // Predict connections (would use actual metrics - simplified for now)
          final predictedConnections = (prediction.predictionScore * 10).round();
          ...
          final predictedTransformativeImpact = prediction.predictionScore * 0.8;
...
class MeaningfulConnectionPredictions {
  ...
  Map<String, dynamic> toJson() {
    return {
      'event_id': eventId,
      'predicted_meaningful_connections': predictedConnections.map((c) => c.toJson()).toList(),
...
class VibeEvolutionPredictions {
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'predicted_vibe_after_events': predictedVibeAfterEvents.map((v) => v.toJson()).toList(),
    };
  }
}
...
class UserJourneyPredictions {
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'predicted_journey_trajectory': predictedTrajectory.map((s) => s.toJson()).toList(),
    };
  }
}
