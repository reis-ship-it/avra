...
class QuantumMatchingProductionService {
    ...
    try {
      ...
      if (_circuitBreakerState == CircuitBreakerState.open) {
        if (_shouldAttemptHalfOpen()) {
          ...
          _logger.info('Circuit breaker entering half-open state', tag: _logName);
        } else {
          _logger.warn('Circuit breaker is open, rejecting request', tag: _logName);
      ...
      if (!_checkRateLimit(operationKey)) {
        _logger.warn('Rate limit exceeded for operation: $operationKey', tag: _logName);
      ...
      if (useCache) {
        ...
        if (cached != null) {
          _logger.debug('Cache hit for operation: $operationKey', tag: _logName);
          return cached;
        }
      }
      ...
      if (_circuitBreakerState == CircuitBreakerState.halfOpen) {
        ...
        _logger.info('Circuit breaker closed after successful operation', tag: _logName);
  ...
  bool _shouldAttemptHalfOpen() {
    ...
    return DateTime.now().difference(_circuitBreakerOpenedAt!) >= _circuitBreakerTimeout;
  }
  ...
  Future<HealthCheckResult> performHealthCheck() async {
    try {
      ...
      checks['circuitBreaker'] = _circuitBreakerState != CircuitBreakerState.open;
      ...
      _logger.error('Health check failed', error: e, stackTrace: st, tag: _logName);
      return HealthCheckResult(
        isHealthy: false,
