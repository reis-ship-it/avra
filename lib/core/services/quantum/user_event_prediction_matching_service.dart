...
class UserEventPredictionMatchingService {
  ...
  Future<UserEventPrediction> predictUserEventCompatibility({
    ...
    try {
      ...
      return UserEventPrediction(
        ...
        reasoning: 'Based on quantum entanglement, knot string predictions, fabric stability, and similar users',
        tAtomic: tAtomic,
      );
    } catch (e, stackTrace) {
      developer.log(
  ...
  Future<double> _calculateKnotStringPrediction({
    ...
    try {
      ...
      for (final similarUser in similarUsers) {
        try {
          final similarAgentId = await _agentIdService.getUserAgentId(similarUser.userId);
      ...
      if (similarUsers.isNotEmpty) {
        try {
          final similarAgentId = await _agentIdService.getUserAgentId(similarUsers.first.userId);
          final trajectory = await _futureCompatService.predictFutureCompatibility(
            userAId: agentId,
          ...
          if (trajectory.isImproving && trajectory.predictedCompatibility > 0.0) {
  ...
  Future<double> _calculatePersonalizedFabricSuitability({
    ...
    try {
      ...
      final userPersonality = await _personalityLearning.getCurrentPersonality(userId);
      ...
      for (final composition in _generateFabricCompositions(
        ...
        try {
          ...
          for (final attendeeId in composition) {
            try {
              final attendeeAgentId = await _agentIdService.getUserAgentId(attendeeId);
          ...
          final fabricQuantumStates = [userQuantumState, ...compositionQuantumStates];
          final fabricEntangledState = await _entanglementService.createEntangledState(
          ...
          final quantumCompatibility = await _entanglementService.calculateFidelity(
            userEntangledState,
          ...
          final knotCompatibility = await _entanglementService.calculateKnotCompatibilityBonus(
            fabricQuantumStates,
          );
          ...
          final globalStability = await _fabricService.measureFabricStability(fabric);
          ...
          try {
            ...
            if (worldsheet != null) {
              ...
              final fabricAtEventTime = worldsheet.getFabricAtTime(event.startTime);
              if (fabricAtEventTime != null) {
                ...
                if (preEventFabric != null && postEventFabric != null) {
                  ...
                  worldsheetEvolutionScore = (0.5 + stabilityChange).clamp(0.0, 1.0);
                }
              }
            }
          } catch (e) {
  ...
  Iterable<List<String>> _generateFabricCompositions({
    ...
    for (var size = 1; size <= similarUsers.length && count < maxCompositions; size++) {
  ...
  double _calculateTopologicalCompatibility({
    ...
    try {
      ...
      final maxCrossings = knotA.invariants.crossingNumber + knotB.invariants.crossingNumber;
      final crossingSimilarity = maxCrossings > 0
          ? 1.0 - (crossingDiff / maxCrossings).clamp(0.0, 1.0)
          : 0.5;

      // Use writhe similarity
      final writheDiff = (knotA.invariants.writhe - knotB.invariants.writhe).abs();
      final writheSimilarity = writheDiff < 0.1 ? 1.0 : (1.0 - writheDiff.clamp(0.0, 1.0));
  ...
  Future<double> _calculateEventOverlap(ExpertiseEvent targetEvent) async {
    try {
      ...
      for (final otherEvent in allEventsList) {
        ...
        // Calculate overlap
        final bothAttended = targetAttendees.intersection(otherAttendees).length;
  ...
  Future<List<SimilarUser>> _findSimilarUsers(
    ...
    try {
      ...
      final targetUserEvents = await _eventService.getEventsByAttendee(targetUser);
      ...
      for (final attendeeId in eventAttendees) {
        ...
        try {
          ...
          final attendeeEvents = await _eventService.getEventsByAttendee(attendee);
          final attendeeEventIds = attendeeEvents.map((e) => e.id).toSet();

          // Calculate behavior pattern similarity (event overlap)
          final commonEvents = targetEventIds.intersection(attendeeEventIds).length;
          final totalEvents = targetEventIds.union(attendeeEventIds).length;
          final behaviorSimilarity = totalEvents > 0 ? commonEvents / totalEvents : 0.0;
      ...
      similarUsers.sort((a, b) => b.similarityWeight.compareTo(a.similarityWeight));
  ...
  Future<double> _createHypotheticalQuantumState({
    ...
    try {
      ...
      final userPersonality = await _personalityLearning.getCurrentPersonality(user.id);
      if (userPersonality == null) {
        developer.log('User personality not found for ${user.id}, using defaults', name: _logName);
        return 0.0;
      }
      ...
      for (final similarUser in similarUsers) {
        try {
          final similarPersonality = await _personalityLearning.getCurrentPersonality(
            similarUser.userId,
          );
          if (similarPersonality == null) {
            developer.log('Personality not found for ${similarUser.userId}, skipping', name: _logName);
            continue;
          }
  ...
  double _calculateLocationSimilarity({
    ...
    if (targetLocation != null && otherLocation != null) {
      ...
      return targetLocation == otherLocation ? 1.0 : 0.3; // Same location = 1.0, different = 0.3
    }
