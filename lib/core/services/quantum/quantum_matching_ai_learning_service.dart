...
class QuantumMatchingAILearningService {
  ...
  Future<PersonalityProfile> learnFromSuccessfulMatch({
    ...
    try {
      ...
      final dimensionInsights = await _extractPersonalityInsights(matchingResult);

      if (dimensionInsights.isEmpty) {
        _logger.debug('No personality insights extracted from match', tag: _logName);
  ...
  Future<MatchingResult?> performOfflineMatching({
    ...
    try {
      ...
      final compatibility = _calculateOfflineCompatibility(entities, cachedStates);
  ...
  Future<void> syncOfflineMatches(String userId) async {
    try {
      ...
      for (final match in offlineMatches) {
        ...
        final dimensionInsights = await _extractPersonalityInsights(match.result);
  ...
  Future<Map<String, double>> _extractPersonalityInsights(MatchingResult result) async {
    final insights = <String, double>{};

    // Base insight from overall compatibility
    final baseInsight = (result.compatibility - 0.5) * 0.2; // Scale to [-0.1, 0.1]
    ...
    if (result.knotCompatibility != null) {
      ...
      insights['social_connection'] = (insights['social_connection'] ?? 0.0) +
          knotInsight;
      insights['authenticity'] = (insights['authenticity'] ?? 0.0) + knotInsight;
    }
    ...
    if (_stringService != null || _fabricService != null || _worldsheetService != null) {
      try {
        ...
        if (result.knotCompatibility != null && result.knotCompatibility! > 0.7) {
          // High knot compatibility suggests strong string/fabric alignment
          final enhancedInsight = (result.knotCompatibility! - 0.7) * 0.08; // Extra boost for high compatibility
          insights['social_connection'] = (insights['social_connection'] ?? 0.0) + enhancedInsight;
          insights['community_engagement'] = (insights['community_engagement'] ?? 0.0) + enhancedInsight;
        }
      } catch (e) {
        _logger.debug('Error extracting string/fabric/worldsheet insights: $e', tag: _logName);
      }
    }
    ...
    for (final dimension in [
      ...
      insights[dimension] = (insights[dimension] ?? 0.0) + baseInsight + quantumInsight;
    }
  ...
  double _calculateLearningQuality(MatchingResult result) {
    var quality = 0.4 * result.compatibility + 0.3 * result.quantumCompatibility;
  ...
  Future<void> _propagateBatchedInsights() async {
    ...
    try {
      final insightsToPropagate = List<_PendingLearningInsight>.from(_pendingInsights);
      _pendingInsights.clear();
  ...
  Future<void> _propagateLearningInsightThroughMesh(
    ...
    try {
      ...
      final payload = <String, dynamic>{
        ...
        'meaningful_connection': pending.matchingResult.meaningfulConnectionScore,
  ...
  Future<void> _cacheQuantumStates(List<QuantumEntityState> entities) async {
    try {
      ...
      await _storageService.setString(_quantumStateCacheKey, jsonEncode(cached));
    } catch (e) {
