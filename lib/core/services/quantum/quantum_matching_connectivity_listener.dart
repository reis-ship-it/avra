...
class QuantumMatchingConnectivityListener {
  ...
  Future<void> start() async {
    ...
    _logger.info('Starting quantum matching connectivity listener', tag: _logName);
    ...
    _connectivitySubscription = _connectivityService.internetAccessStream().listen(
      (isOnline) {
  ...
  void stop() {
    ...
    _logger.info('Stopped quantum matching connectivity listener', tag: _logName);
  }
  ...
  Future<void> _handleConnectivityChange(bool isOnline) async {
    try {
      ...
      if (_wasOffline && isOnline) {
        _logger.info('Device came back online, syncing offline matches', tag: _logName);
  ...
  Future<void> _syncOfflineMatches() async {
    if (_aiLearningService == null) {
      _logger.debug('AI learning service not available, skipping sync', tag: _logName);
      return;
    }
      ...
      if (sl.isRegistered<SupabaseService>()) {
        ...
        if (supabaseService.isAvailable) {
          ...
          if (currentUser != null) {
            ...
            _logger.info('Synced offline matches for user: ${currentUser.id}', tag: _logName);
          } else {
            _logger.debug('No authenticated user, skipping offline match sync', tag: _logName);
          }
        }
      }
    } catch (e, stackTrace) {
