...
class QuantumKnotVibeCompatibilityService implements VibeCompatibilityService {
  ...
  Map<String, double> _ensureAllDimensions(Map<String, double> input) {
    ...
    for (final d in VibeConstants.coreDimensions) {
      dims[d] = (input[d] ?? VibeConstants.defaultDimensionValue).clamp(0.0, 1.0);
    }
    return dims;
  }
  ...
  Map<String, double>? _extractExplicitVibeDimensions(
    ...
    final dynamic raw = preferences['vibeDimensions'] ?? preferences['vibe_dimensions'];
  ...
  Future<EntityKnot?> _tryEntityKnotForPerson(PersonalityProfile profile) async {
    try {
      // Prefer embedded knot if present, otherwise generate.
      final knot = profile.personalityKnot ?? await _personalityKnotService.generateKnot(profile);
      return EntityKnot(
  ...
  Future<EntityKnot?> _tryEntityKnotForBusiness(BusinessAccount business) async {
    try {
