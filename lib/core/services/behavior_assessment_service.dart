...
class BehaviorAssessmentService {
  ...
  BehaviorProfile assessBehavior(
    ...
    final socialContext = context?['socialContext'] as String?; // 'solo', 'family', 'friends', 'supervised'
    final purpose = context?['purpose'] as String?; // 'education', 'entertainment', 'social', 'work'
    ...
    var educationalValue = _assessEducationalValue(behaviorType, purpose, context);
    ...
    final physicalActivityValue = _assessPhysicalActivityValue(behaviorType, context);
    ...
    final adjustments = _calculateVenueQualityAdjustments(behaviorType, venueQuality);
    
    // Apply venue quality adjustments
    developmentalAppropriateness = (developmentalAppropriateness + adjustments.developmental).clamp(0.0, 1.0);
    educationalValue = (educationalValue + adjustments.educational).clamp(0.0, 1.0);
  ...
  double _assessDevelopmentalAppropriateness(
    ...
    if (_isAdultOriented(type)) {
      ...
      if (age >= 18 && socialContext == 'supervised') return 0.4; // Moderate with supervision
      if (age >= 18) return 0.2; // Low appropriateness
      if (age >= 16 && purpose == 'education' && socialContext == 'supervised') return 0.3; // Educational context
  ...
  double _assessEducationalValue(
    ...
    if (purpose == 'education' || type.contains('museum') || type.contains('library') ||
        type.contains('school') || type.contains('class') || type.contains('workshop')) {
      return 0.9;
    }
    
    // Culturally enriching
    if (type.contains('orchestra') || type.contains('symphony') || type.contains('theater') ||
        type.contains('gallery') || type.contains('art') || type.contains('music') ||
        type.contains('cultural') || type.contains('exhibition')) {
      return 0.8;
    }
    
    // Social learning
    if (type.contains('community') || type.contains('volunteer') || type.contains('service')) {
      return 0.7;
    }
  ...
  double _assessSocialValue(
    ...
    if (type.contains('community') || type.contains('volunteer') || type.contains('team') ||
        type.contains('group') || type.contains('club') || type.contains('social')) {
      return 0.9;
    }
    
    // Social venues (context-dependent)
    if (type.contains('bar') || type.contains('pub') || type.contains('cafe') ||
        type.contains('restaurant') || type.contains('lounge')) {
  ...
  double _assessRiskLevel(
    ...
    if (type.contains('casino') || type.contains('gambling') || type.contains('extreme') ||
        type.contains('dangerous')) {
      return 0.9; // High risk
    }
    
    // Alcohol-related (risk increases for younger ages)
    if (type.contains('bar') || type.contains('pub') || type.contains('alcohol') ||
        type.contains('drinking') || type.contains('brewery') || type.contains('wine')) {
    ...
    if (type.contains('nightclub') || type.contains('nightlife') || type.contains('club')) {
    ...
    // Low-risk activities
    if (type.contains('museum') || type.contains('library') || type.contains('park') ||
        type.contains('educational') || type.contains('cultural')) {
  ...
  double _assessCulturalValue(String behaviorType, Map<String, dynamic>? context) {
    final type = behaviorType.toLowerCase();
    
    if (type.contains('museum') || type.contains('gallery') || type.contains('art') ||
        type.contains('orchestra') || type.contains('symphony') || type.contains('theater') ||
        type.contains('cultural') || type.contains('heritage') || type.contains('history')) {
      return 0.9;
    }
    
    if (type.contains('music') || type.contains('dance') || type.contains('performance') ||
        type.contains('festival') || type.contains('exhibition')) {
      return 0.7;
    }
  ...
  double _assessPhysicalActivityValue(String behaviorType, Map<String, dynamic>? context) {
    final type = behaviorType.toLowerCase();
    
    if (type.contains('sport') || type.contains('fitness') || type.contains('exercise') ||
        type.contains('gym') || type.contains('hiking') || type.contains('running') ||
        type.contains('swimming') || type.contains('cycling') || type.contains('outdoor')) {
      return 0.9;
    }
    
    if (type.contains('walking') || type.contains('park') || type.contains('outdoor')) {
      return 0.6;
    }
  ...
  bool _isEducationalOrCultural(String type, String? purpose) {
    return type.contains('museum') || type.contains('library') || type.contains('school') ||
           type.contains('orchestra') || type.contains('symphony') || type.contains('theater') ||
           type.contains('gallery') || type.contains('art') || type.contains('cultural') ||
           type.contains('education') || type.contains('class') || type.contains('workshop') ||
           purpose == 'education';
  }
  
  bool _isPhysicalActivity(String type) {
    return type.contains('sport') || type.contains('fitness') || type.contains('exercise') ||
           type.contains('gym') || type.contains('hiking') || type.contains('outdoor') ||
           type.contains('park') || type.contains('swimming') || type.contains('cycling');
  }
  
  bool _isSocialActivity(String type) {
    return type.contains('social') || type.contains('community') || type.contains('group') ||
           type.contains('team') || type.contains('club') || type.contains('event');
  }
  
  bool _isAdultOriented(String type) {
    return type.contains('bar') || type.contains('pub') || type.contains('nightclub') ||
           type.contains('lounge') || type.contains('alcohol') || type.contains('drinking') ||
           type.contains('brewery') || type.contains('wine') || type.contains('casino') ||
           type.contains('adult') || type.contains('nightlife');
  }
  ...
  VenueQualityProfile _assessVenueQuality(Map<String, dynamic>? context) {
    ...
    final priceLevel = context['priceLevel'] as String?; // 'low', 'moderate', 'high', 'premium'
    ...
    final sophistication = _assessSophistication(rating, priceLevel, tags, description, name, metadata);
    ...
    final culturalSophistication = _assessCulturalSophistication(tags, description, name, metadata);
  ...
  double _assessSophistication(
    ...
    if (priceLevel != null) {
      switch (priceLevel.toLowerCase()) {
        case 'premium':
        case 'high':
          score += 0.2; // Slight boost (not "better", just different connection potential)
          break;
        case 'moderate':
          score += 0.0; // Neutral
          break;
    ...
    if (tags != null) {
      ...
      if (tagStrings.any((t) => t.contains('art') || t.contains('cultural') ||
                                t.contains('literary') || t.contains('intellectual') ||
                                t.contains('community') || t.contains('welcoming') ||
                                t.contains('conversation') || t.contains('networking'))) {
        score += 0.4; // Higher connection potential
      }
      
      // Lower connection potential indicators (but not "bad")
      if (tagStrings.any((t) => t.contains('dive') || t.contains('grungy') ||
                                t.contains('seedy') || t.contains('sketchy') ||
                                t.contains('rough'))) {
        score -= 0.2; // Lower connection potential (different type of connections)
      }
    }
    
    // Description/name analysis - look for connection potential indicators
    final combinedText = '$name $description'.toLowerCase();
    if (combinedText.contains('art') || combinedText.contains('gallery') ||
        combinedText.contains('cultural') || combinedText.contains('literary') ||
        combinedText.contains('intellectual') || combinedText.contains('conversation') ||
        combinedText.contains('community') || combinedText.contains('welcoming')) {
      score += 0.3; // Higher connection potential
    }
    
    if (combinedText.contains('dive') || combinedText.contains('grungy') ||
        combinedText.contains('seedy') || combinedText.contains('sketchy')) {
  ...
  double _assessAmbiance(
    ...
    if (combinedText.contains('welcoming') || combinedText.contains('cozy') ||
        combinedText.contains('conversation') || combinedText.contains('community') ||
        combinedText.contains('intimate') || combinedText.contains('relaxed') ||
        tagStrings.any((t) => t.contains('welcoming') || t.contains('cozy') ||
                             t.contains('conversation') || t.contains('community'))) {
    ...
    if (combinedText.contains('grungy') || combinedText.contains('seedy') ||
        combinedText.contains('rough') || combinedText.contains('sketchy') ||
        combinedText.contains('dive') || combinedText.contains('loud') ||
        tagStrings.any((t) => t.contains('grungy') || t.contains('seedy') ||
                             t.contains('rough') || t.contains('loud'))) {
  ...
  double _assessCulturalSophistication(
    ...
    double score = 0.3; // Start lower (most venues aren't culturally sophisticated)
    ...
    if (combinedText.contains('art') || combinedText.contains('gallery') ||
        combinedText.contains('music') || combinedText.contains('jazz') ||
        combinedText.contains('orchestra') || combinedText.contains('symphony') ||
        combinedText.contains('literary') || combinedText.contains('book') ||
        combinedText.contains('cultural') || combinedText.contains('heritage') ||
        combinedText.contains('intellectual') || combinedText.contains('conversation') ||
        tagStrings.any((t) => t.contains('art') || t.contains('cultural') ||
                             t.contains('music') || t.contains('literary') ||
                             t.contains('intellectual') || t.contains('conversation'))) {
      score += 0.5; // Significant boost - higher probability of meaningful connections
    }
    ...
    if (name.toLowerCase().contains('bemelmans') || 
        ...
        combinedText.contains('murals') || combinedText.contains('artwork')) {
  ...
  double _assessSafetyLevel(
    ...
    // Safety indicators
    if (combinedText.contains('safe') || combinedText.contains('welcoming') ||
        combinedText.contains('family') || combinedText.contains('upscale') ||
        tagStrings.any((t) => t.contains('safe') || t.contains('welcoming'))) {
      score += 0.2;
    }
    
    // Risk indicators
    if (combinedText.contains('sketchy') || combinedText.contains('rough') ||
        combinedText.contains('seedy') || combinedText.contains('dive') ||
        combinedText.contains('grungy') || combinedText.contains('dangerous') ||
        tagStrings.any((t) => t.contains('sketchy') || t.contains('rough') ||
                             t.contains('seedy') || t.contains('dangerous'))) {
      score -= 0.4;
    }
  ...
  VenueQualityAdjustments _calculateVenueQualityAdjustments(
    ...
    if (venueQuality.sophistication >= 0.7) {
      return VenueQualityAdjustments(
        ...
        educational: venueQuality.culturalSophistication * 0.3, // Boost - meaningful connections are educational
        social: 0.1, // Better environment for connections
        risk: -(1.0 - venueQuality.safetyLevel) * 0.2, // Reduce risk (negative = lower risk)
        cultural: venueQuality.culturalSophistication * 0.4, // Significant boost - life-changing potential
      );
    }
    ...
    if (venueQuality.sophistication <= 0.3) {
      return VenueQualityAdjustments(
        ...
        educational: -0.1, // Lower educational value (different connection type)
  ...
  double calculateLearningFilter(
    ...
    Map<String, double>? learnerPersonality, // User's personality profile for individual matching
    List<String>? learnerPreferences, // User's preferences for trajectory matching
    Map<String, dynamic>? learnerHistory, // User's history of what works for them
  }) {
    ...
    if (learnerPersonality != null || learnerPreferences != null || learnerHistory != null) {
  ...
  double _calculateIndividualTrajectoryAdjustment(
    ...
    if (learnerPersonality != null) {
      ...
      final explorationEagerness = learnerPersonality['exploration_eagerness'] ?? 0.5;
      final authenticityPreference = learnerPersonality['authenticity_preference'] ?? 0.5;
      final communityOrientation = learnerPersonality['community_orientation'] ?? 0.5;
      ...
      if (authenticityPreference > 0.7) {
        ...
        final venueType = behaviorContext?['name']?.toString().toLowerCase() ?? '';
        if (venueType.contains('dive') || venueType.contains('authentic') || 
      ...
      if (communityOrientation > 0.7) {
        final venueType = behaviorContext?['name']?.toString().toLowerCase() ?? '';
        if (venueType.contains('community') || venueType.contains('local') ||
    ...
    if (learnerPreferences != null) {
      final behaviorType = behaviorContext?['behaviorType']?.toString().toLowerCase() ?? '';
      final venueName = behaviorContext?['name']?.toString().toLowerCase() ?? '';
      final venueDescription = behaviorContext?['description']?.toString().toLowerCase() ?? '';
    ...
    if (learnerHistory != null) {
      final successfulConnections = learnerHistory['successful_connections'] as List<dynamic>?;
      final positiveExperiences = learnerHistory['positive_experiences'] as List<dynamic>?;
...
class VenueQualityProfile {
  final double sophistication; // 0.0 to 1.0 (higher = higher connection potential)
  final double ambiance; // 0.0 to 1.0 (higher = better environment for connections)
  final double culturalSophistication; // 0.0 to 1.0 (higher = higher life-changing potential)
  final double safetyLevel; // 0.0 to 1.0 (higher = safer, more conducive to connections)
  ...
  double get overallQuality {
    return (
      sophistication * 0.30 +  // Connection potential
