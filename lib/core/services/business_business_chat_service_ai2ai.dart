...
import 'package:spots/core/models/business_business_message.dart';
...
class BusinessBusinessChatServiceAI2AI {
  ...
  Future<BusinessBusinessMessage> sendMessage({
    ...
    try {
      ...
      final conversation = await _getOrCreateConversation(senderBusinessId, recipientBusinessId);
        ...
        if (message.encryptedContent != null) {
          try {
            ...
            final decrypted = await _encryptionService.decrypt(
              encrypted,
              message.senderBusinessId, // Use sender ID for decryption key lookup
            );
  ...
  Future<List<BusinessBusinessMessage>> _checkForNewMessages(String conversationId) async {
