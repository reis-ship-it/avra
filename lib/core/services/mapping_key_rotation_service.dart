...
class MappingKeyRotationService {
  ...
  Future<void> rotateUserKey(String userId) async {
    try {
      ...
      developer.log('🔄 Rotating encryption key for user: $userId', name: _logName);
      ...
      try {
        await client.rpc('increment_rotation_count', params: {'p_user_id': userId});
      } catch (e) {
      ...
      developer.log('✅ Key rotation complete for user: $userId', name: _logName);
    } catch (e, stackTrace) {
      developer.log(
      ...
      for (int batch = 0; batch < batches; batch++) {
        ...
        for (final userId in batchUserIds) {
          try {
            ...
            if (_isRateLimited(userId)) {
              skipped++;
              developer.log('⏭️  Skipping $userId (rate limited)', name: _logName);
              continue;
            }
  ...
  Future<RotationBatchResult> rotateAllKeys({
    ...
    try {
      ...
      final allUserIds = (response as List)
          .map((e) => e['user_id'] as String)
          .toList();
      
      final effectiveUserIds = limit != null
          ? allUserIds.take(limit).toList()
          : allUserIds;
