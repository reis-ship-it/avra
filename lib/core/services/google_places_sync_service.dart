...
class GooglePlacesSyncService {
  ...
  Future<Spot?> syncSpot(Spot spot) async {
    try {
      ...
      final isOnline = connectivityResult.contains(ConnectivityResult.none) == false;
      ...
      if (placeId == null) {
        developer.log('No Google Place ID found for: ${spot.name}', name: _logName);
      ...
      final googlePlace = await _googlePlacesDataSource.getPlaceDetails(placeId);
      if (googlePlace == null) {
        developer.log('Could not get Google Place details for: $placeId', name: _logName);
      ...
      developer.log('Successfully synced spot: ${spot.name} with Place ID: $placeId', name: _logName);
      return syncedSpot;
    } catch (e) {
  ...
  Future<SyncResult> syncSpots(List<Spot> spots, {int batchSize = 10}) async {
    try {
      ...
      developer.log('Sync complete: ${results.synced} synced, ${results.failed} failed', name: _logName);
      return results;
    } catch (e) {
  ...
  Future<SyncResult> syncSpotsNeedingSync({int limit = 50}) async {
    try {
      ...
      developer.log('Found ${spotsToSync.length} spots that need syncing', name: _logName);
