...
class LedgerReceiptsServiceV0 {
  ...
  LedgerReceiptsServiceV0({
    ...
    AppLogger? logger,
        ...
        _logger = logger ??
            const AppLogger(defaultTag: 'Ledgers', minimumLevel: LogLevel.debug);
  ...
  Future<List<LedgerReceiptV0>> listReceipts({
    ...
    try {
      ...
      developer.log('Failed listing receipts', error: e, stackTrace: st, name: _logName);
      return const [];
    }
  }
  ...
  Future<LedgerReceiptV0?> getReceiptByLedgerRowId(String ledgerRowId) async {
    ...
    try {
      ...
      return LedgerReceiptV0.fromSupabaseRow((row as Map).cast<String, dynamic>());
    } catch (e, st) {
      developer.log('Failed reading receipt', error: e, stackTrace: st, name: _logName);
      return null;
    }
  }
  ...
  Future<LedgerReceiptSignatureV0> signExisting({
    ...
    if (response.status != 200) {
      throw Exception('sign_existing failed: ${response.status} ${response.data}');
    }
  ...
  Future<bool> verifyReceipt(LedgerReceiptV0 receipt) async {
    try {
      ...
      developer.log('Receipt verification failed', error: e, stackTrace: st, name: _logName);
      return false;
    }
  }
}
