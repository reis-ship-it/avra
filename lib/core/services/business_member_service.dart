  ...
  Future<BusinessMember> addMember({
    ...
    try {
      ...
      final inviterMember = business.members.firstWhere(
        ...
        orElse: () => throw Exception('Inviter is not a member of this business'),
      );
  ...
  Future<bool> removeMember({
    ...
    try {
      ...
      final memberToRemove = business.members.firstWhere(
        (m) => m.userId == userId && m.isActive,
        orElse: () => throw Exception('User is not an active member of this business'),
      );
      ...
      final removerMember = business.members.firstWhere(
        ...
        orElse: () => throw Exception('Remover is not a member of this business'),
      );
      ...
      _logger.error('Error removing member from business', error: e, tag: _logName);
      rethrow;
    }
  }
  ...
  Future<BusinessMember> updateMemberRole({
    ...
    try {
      ...
      final updaterMember = business.members.firstWhere(
        ...
        orElse: () => throw Exception('Updater is not a member of this business'),
      );
      ...
      if (memberToUpdate.role == BusinessMemberRole.owner && newRole != BusinessMemberRole.owner) {
      ...
      _logger.info('Updated member role: $userId -> ${newRole.name}', tag: _logName);
    ...
    try {
      ...
      switch (requiredRole) {
        ...
        case BusinessMemberRole.admin:
          return member.role == BusinessMemberRole.admin || member.role == BusinessMemberRole.owner;
