...
class WebRTCSignalingConfig {
  ...
  static const String _defaultSignalingServerUrl = 'wss://signaling.spots.app';
  ...
  String getSignalingServerUrl() {
    if (!kIsWeb) {
      developer.log('WebRTC signaling only available on Web platform', name: _logName);
      return '';
    }
  ...
  Future<bool> setSignalingServerUrl(String url) async {
    ...
    try {
      ...
      developer.log('SharedPreferences not available, URL not persisted', name: _logName);
      return false;
    } catch (e) {
