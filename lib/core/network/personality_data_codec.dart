...
class PersonalityDataCodec {
  ...
  static const List<int> _spotsMagicBytes = [0x53, 0x50, 0x4F, 0x54, 0x53]; // "SPOTS"
  ...
  static List<int> encodeToBinary(AnonymizedVibeData data) {
    try {
      developer.log('Encoding personality data to binary format', name: _logName);
      ...
      developer.log('Encoded ${jsonBytes.length} bytes of personality data', name: _logName);
      return encoded;
    } catch (e) {
  ...
  static AnonymizedVibeData? decodeFromBinary(List<int> data) {
    try {
      if (data.length < _spotsMagicBytes.length + 2) {
        developer.log('Data too short to contain personality data', name: _logName);
        return null;
      }
      ...
      developer.log('Error decoding binary personality data: $e', name: _logName);
      return null;
    }
  }
  ...
  static String encodeToBase64(AnonymizedVibeData data) {
    try {
      ...
      developer.log('Encoded personality data to ${base64String.length} base64 characters', name: _logName);
      return base64String;
    } catch (e) {
  ...
  static AnonymizedVibeData? decodeFromBase64(String base64String) {
    try {
      ...
      developer.log('Error decoding base64 personality data: $e', name: _logName);
      return null;
    }
  }
  ...
  static AnonymizedVibeData _jsonToAnonymizedVibeData(Map<String, dynamic> json) {
    // Parse anonymized metrics
    final metricsJson = json['anonymized_metrics'] as Map<String, dynamic>;
    final metrics = AnonymizedVibeMetrics(
      energy: (metricsJson['energy'] as num).toDouble(),
      social: (metricsJson['social'] as num).toDouble(),
      exploration: (metricsJson['exploration'] as num).toDouble(),
    );
    
    // Parse noisy dimensions
