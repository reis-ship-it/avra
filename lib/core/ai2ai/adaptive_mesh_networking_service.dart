...
class AdaptiveMeshNetworkingService {
  ...
  int? _currentMaxHops;
  final mesh_policy.MessagePriority _defaultPriority = mesh_policy.MessagePriority.medium;
  final mesh_policy.MessageType _defaultMessageType = mesh_policy.MessageType.learningInsight;
  ...
  bool shouldForwardMessage({
    ...
    String? geographicScope,  // NEW: "locality", "city", "region", "country", "global"
  }) {
    // Always allow direct connections (0 hops)
    if (currentHop == 0) return true;
    
    // NEW: If sender is expert, allow more hops
    if (senderExpertise != null && senderExpertise.index >= ExpertiseLevel.city.index) {
