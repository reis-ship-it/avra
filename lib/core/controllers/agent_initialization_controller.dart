...
/// }
/// ```
class AgentInitializationController implements WorkflowController<Map<String, dynamic>, AgentInitializationResult> {
  ...
  AgentInitializationController({
    ...
    SocialMediaConnectionService? socialMediaService, // Deprecated - kept for backward compatibility
  ...
  }) : _socialMediaDataController = socialMediaDataController ?? GetIt.instance<SocialMediaDataCollectionController>(),
       _personalityLearning = personalityLearning ?? GetIt.instance<PersonalityLearning>(),
       _preferencesService = preferencesService ?? GetIt.instance<PreferencesProfileService>(),
       _placeListGenerator = placeListGenerator ?? GetIt.instance<OnboardingPlaceListGenerator>(),
       _recommendationService = recommendationService ?? GetIt.instance<OnboardingRecommendationService>(),
  ...
  Future<AgentInitializationResult> initializeAgent({
    ...
    try {
      ...
      try {
        ...
        final collectionResult = await _socialMediaDataController.collectAllData(userId: userId);
        
        if (collectionResult.isSuccess && collectionResult.structuredData != null) {
          ...
          if (collectionResult.platformErrors != null && collectionResult.platformErrors!.isNotEmpty) {
            _logger.warn(
          ...
          _logger.debug('ℹ️ No social media data collected (no connections or all failed)', tag: _logName);
        }
      } catch (e) {
        _logger.warn('⚠️ Could not collect social media data: $e', tag: _logName);
      ...
      try {
        ...
        personalityProfile = await _personalityLearning.initializePersonalityFromOnboarding(
          userId,
        ...
        _logger.error('❌ Failed to initialize PersonalityProfile: $e', error: e, tag: _logName);
        return AgentInitializationResult.failure(
      ...
      try {
        ...
        _logger.warn('⚠️ Could not initialize PreferencesProfile: $e', tag: _logName);
      ...
      if (generatePlaceLists) {
        try {
          ...
          if (homebaseForPlaces.isNotEmpty) {
            ...
            generatedPlaceLists = placeLists.map((list) => {
              'name': list.name,
            ...
            _logger.info('📍 Generated ${placeLists.length} place lists', tag: _logName);
          }
        } catch (e) {
      ...
      if (getRecommendations) {
        try {
          ...
          final recommendedLists = await _recommendationService.getRecommendedLists(
            userId: userId,
            onboardingData: onboardingDataMap,
          ...
          final recommendedAccounts = await _recommendationService.getRecommendedAccounts(
            userId: userId,
            onboardingData: onboardingDataMap,
            personalityDimensions: personalityProfile.dimensions,
            maxRecommendations: 10,
          ...
          recommendations = {
            'lists': recommendedLists.map((rec) => {
              'listName': rec.listName,
              'compatibilityScore': rec.compatibilityScore,
            }).toList(),
            'accounts': recommendedAccounts.map((rec) => {
      ...
      if (attemptCloudSync) {
        try {
          ...
          if (syncEnabled) {
            ...
            _logger.info('☁️ Cloud sync enabled, attempting to sync profile...', tag: _logName);
            ...
            cloudSyncSucceeded = true; // Mark as attempted, actual sync may happen later
          } else {
            _logger.debug('ℹ️ Cloud sync disabled for user', tag: _logName);
          }
        } catch (e) {
          _logger.warn('⚠️ Error checking cloud sync status: $e', tag: _logName);
          // Continue - don't block initialization
        }
      }
      
      // STEP 8: Return success result
      _logger.info('✅ Agent initialization workflow completed successfully', tag: _logName);
