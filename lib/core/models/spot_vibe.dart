...
class SpotVibe {
  final String spotId;
  final Map<String, double> vibeDimensions; // Same 12 dimensions as user personality
  ...
  double calculateVibeCompatibility(UserVibe userVibe) {
    ...
    final energyCompatibility = 1.0 - (overallEnergy - userVibe.overallEnergy).abs();
    final explorationCompatibility = 1.0 - (explorationTendency - userVibe.explorationTendency).abs();
    final socialCompatibility = 1.0 - (socialPreference - userVibe.socialPreference).abs();
    
    // Weighted average
    final overallCompatibility = (
      dimensionCompatibility * 0.5 +
  ...
  static Map<String, double> _inferDimensionsFromCharacteristics(
    ...
    final combinedText = '$category ${tags.join(' ')} $description'.toLowerCase();
    ...
    if (combinedText.contains('new') || combinedText.contains('unique') || 
        combinedText.contains('hidden') || combinedText.contains('gem')) {
    ...
    if (combinedText.contains('community') || combinedText.contains('group') ||
        combinedText.contains('social') || combinedText.contains('meetup')) {
    ...
    if (combinedText.contains('authentic') || combinedText.contains('local') ||
        combinedText.contains('artisan') || combinedText.contains('craft')) {
      dimensions['authenticity_preference'] = 0.8;
    } else if (combinedText.contains('popular') || combinedText.contains('trendy')) {
    ...
    // Energy preference
    if (combinedText.contains('energetic') || combinedText.contains('lively') ||
        combinedText.contains('vibrant') || combinedText.contains('bustling')) {
      dimensions['energy_preference'] = 0.8;
    } else if (combinedText.contains('chill') || combinedText.contains('relaxed') ||
               combinedText.contains('quiet') || combinedText.contains('calm')) {
    ...
    if (combinedText.contains('art') || combinedText.contains('culture') ||
        combinedText.contains('museum') || combinedText.contains('gallery') ||
        combinedText.contains('sophisticated') || combinedText.contains('refined')) {
  ...
  static String _generateVibeDescription(
    ...
    if (dimensions['energy_preference'] != null && dimensions['energy_preference']! > 0.7) {
      parts.add('high-energy');
    } else if (dimensions['energy_preference'] != null && dimensions['energy_preference']! < 0.3) {
      parts.add('chill');
    }
    
    if (dimensions['community_orientation'] != null && dimensions['community_orientation']! > 0.7) {
      parts.add('community-focused');
    }
    
    if (dimensions['authenticity_preference'] != null && dimensions['authenticity_preference']! > 0.7) {
      parts.add('authentic');
    }
    
    if (tags.contains('art') || tags.contains('cultural') || tags.contains('sophisticated')) {
